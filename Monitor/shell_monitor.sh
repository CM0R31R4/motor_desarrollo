...
#LOG
echo "$HORA_MONITOR;"`date "+%Y%m%d;%T"`";autentia-audit;0;$STATUS_SERVICIO_AUT;$HTTP_CODE;$TIME_TOTAL_AUTENTIA;$TAG_ERR;$RSP_WS">>$LOG

#Enviar a WS Monitor
ws_monitor "1&AUDIT&$HORA_MONITOR&$TIME_TOTAL_AUTENTIA& & & & &$STATUS_SERVICIO_AUT& & & & &$RSP_WS& & & & &"
...
REQ="<soapenv:Envelope xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:xsd=\"http://www.w3.org/2001/XMLSchema\" xmlns:soapenv=\"http://schemas.xmlsoap.org/soap/envelope/\" xmlns:urn=\"urn:wspersona\"><soapenv:Header/><soapenv:Body><urn:wspersona soapenv:encodingStyle=\"http://schemas.xmlsoap.org/soap/encoding/\"><Req xsi:type=\"urn:CPersonaReq\"><wsUsuario xsi:type=\"xsd:string\">Autentia</wsUsuario><wsClave xsi:type=\"xsd:string\">@ut3nti4.</wsClave><Pais>CHILE</Pais><Rut xsi:type=\"xsd:string\">0011223574-4</Rut><Institucion xsi:type=\"xsd:string\">?</Institucion></Req></urn:wspersona></soapenv:Body></soapenv:Envelope>"
...
echo "TIME_TOTAL_AUTENTIA ---->$TIME_TOTAL_AUTENTIA"
RSP_WS=$(get_tag "CPersonaResp" "${RSP}")
echo "RSP_WS----- : $RSP_WS"
...
#LOG
echo "$HORA_MONITOR;"`date "+%Y%m%d;%T"`";autentia-persona;0;$STATUS_SERVICIO_AUT;$HTTP_CODE;$TIME_TOTAL_AUTENTIA;$TAG_ERR;$RSP_WS">>$LOG

#Enviar a WS Monitor
ws_monitor "1&PERSONA&$HORA_MONITOR&$TIME_TOTAL_AUTENTIA& & & & &$STATUS_SERVICIO_AUT& & & & &$RSP_WS& & & & &"
...
#Enviar a WS Monitor
ws_monitor "1&PERCONA&$HORA_MONITOR&$TIME_TOTAL_PERCONA& & & & &$STATUS_SERVICIO_PER& & & & &$RSP_WS& & & & &"
...
do
	...
	do
		...
		echo "ERROR----: $ERROR"
	        ...
	        if [[ $HTTP_CODE == 200 ]] && [[ $ERROR == 0 ]];
	        then
			if [[  $ID == "01"  ]] && [[ $RSP_WS_BEN == *"[CERBenCertif]"* ]];
			then
        ...
        #LOG
        echo "$HORA_MONITOR;"`date "+%Y%m%d;%T"`";bencertif;$ID;$STATUS_SERVICIO_BEN;$HTTP_CODE;$TIME_TOTAL_BENCERTIF;$ESTADO;$MENSAJE;$RSP_WS_BEN">>$LOG
