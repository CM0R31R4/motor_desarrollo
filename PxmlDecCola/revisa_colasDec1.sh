...
# Esto se almacena en hexadecimal para evitar la codificacion
        ...
        /bin/ps -ef|/bin/grep -v grep|/bin/grep "actoVentaXmlCola " | /bin/grep Procesa_ColasDec1 >/dev/null 2>&1
        case "$?" in
           0)
           # Si ya esta corriendo, entonces hace nada.
                echo "actoVentaXml Cola Procesador 1`date  +'%H:%M:%S'` Script ColasDec corriendo..."
           ;;
          1)
		...
		echo "Inicia actoVentaXmlCola Procesador 1."
		cd /home/motor/PxmlDecCola/
		#TODO Este shell solo procesa estados "SOLO_PERCONA"

		/home/motor/PxmlDecCola/Procesa_ColasDec1 portdb=9000 conexionbd=INTERNA usuario_bd="confirma" clave_bd="" log=1 ip_conexion_bd=127.0.0.1 basedatos=dmotor port_postgres=5432 tabla="actoVentaXmlCola" id="1" prefijo_log="XmlColaDec_P1" SP_REINTENTOS_REG="-" SP_BORRA_REG_COLA="-" sp_consume_cola="SELECT id, xml as data FROM tx_actoventaxml WHERE id_cola in (1,2) and reintentos < 3 ORDER BY id limit 10" >/dev/null &
