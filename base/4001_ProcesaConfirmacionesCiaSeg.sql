...
begin
        ...
        xml2:=logapp(xml2,'111Method='||get_campo('REQUEST_METHOD',xml2)||' -- Uri='||get_campo('REQUEST_URI',xml2)||' -- Host='||get_campo('HTTP_HOST',xml2));
        if (get_campo('REQUEST_METHOD',xml2)='GET' and strpos(get_campo('REQUEST_URI',xml2),'wsdl')>0) then
                ...
                file_wsdl1:=replace(file_wsdl1,chr(36)||chr(36)||'HOST'||chr(36)||chr(36),get_campo('HTTP_HOST',xml2));
...
/*
        -- Inicio modificacion 2015-06-30. - Se agrega nuevo formato de creacion de tabla tx_cias.
        -- Asigna valor del dia.
        fecha_tabla1:=to_char(now(),'YYYYMMDD');

        -- Se le agrega codigo del motor. Por mientras se realiza en duro.
        -- El numero 1 al final corresponde al numero de la maquina.
        fecha_tabla1:=(fecha_tabla1 * 100) + 1;

        -- Se compone el nombre final de la tabla.
        tx_tabla1:=('tx_cias_'||fecha_tabla1)::varchar;
*/
...
/*                if (tx1 in ('Confirmacion','ConfirmacionBono3')) then
                        input1:=decode(get_campo('INPUT',xml2),'hex');
                        xml2:=logapp(xml2,'JCC_extFolioBono='||get_xml2('extFolioBono',input1)||' extRutBeneficiario='||get_xml2('extRutBeneficiario',input1)||'extFolioAuto='||get_campo('extFolioAuto',xml2));
                        --extFolioBono1:=(select extfoliobono from tx_cias where extfolioauto=extFolioAuto1);
                        --extFolioBono1:=(EXECUTE 'SELECT extfoliobono FROM '||tx_tabla1||' WHERE extfolioauto = '||extFolioAuto1||';');
                        execute 'SELECT extfoliobono FROM '||tx_tabla1||' WHERE extfolioauto = '||extFolioAuto1 into extFolioBono1;
                ...
                --Si es una Anulacion
                        ...
                        if (extRutBeneficiario1='000000000000') then
                                ...
                                --extRutBeneficiario1=(execute 'SELECT rut_beneficiario FROM '||tx_tabla1||' WHERE num_operacion IN (SELECT num_operacion FROM '||tx_tabla1||' WHERE extfolioauto='||extFolioAuto1||' AND tx=''Confirmacion'' AND codigo_cia=get_campo(''CODIGO_CIA'',xml2)) AND tx=''Certificacion'' limit 1');
                                execute 'SELECT rut_beneficiario FROM '||tx_tabla1||' WHERE num_operacion IN (SELECT num_operacion FROM '||tx_tabla1||' WHERE extfolioauto='||extFolioAuto1||' AND tx=''Confirmacion'' AND codigo_cia=get_campo(''CODIGO_CIA'',xml2)) AND tx=''Certificacion'' limit 1' into extRutBeneficiario1;
...
*/
	...
	xml2:=logapp(xml2,'F-4001:: Busca en stReq nemo='||stCia.nemo||' tx='||stCia.tx);
        select * into stReq from requerimientos_cia where nemo=stCia.nemo and tx=stCia.tx;
        if not found then
                --Leo una respuesta por defecto
		xml2:=logapp(xml2,'F-4001:: Busca en stReq nemo=defecto');
	...
	xml2:=put_campo(xml2,'COD_FIN',get_xml2('extCodFinanciador',input1));

        if length(stCia.funcion_input)>0 then
                 xml2:=logapp(xml2,'Ejecuta 4001-Funcion = '||stCia.funcion_input);
	...
	-- Se agrega tag para insertar en campo 'request_cia' de tabla tx_cias. [19-11-2015].
	xml2:=put_campo(xml2,'REQUEST_CIA', replace(stReq.data,'&-&DATA&-&',get_campo('DATA_CIA',xml2)));

        --Este tag viene solamente desde sp_input_cbvida
        soapaction1:=get_campo('CIA_SOAPACTION',xml2);
        
	--Cambiamos el SoapAction segun el tipo de tx entrante
        if (tx1 in ('ConfirmacionBono3','AnulacionBono3') and length(soapaction1)=0 ) then
                soapaction1:='http://tempuri.org/wmImed_Srv'||split_part(tx1,'Bono3',1);
                xml2:=put_campo(xml2,'__SECUENCIAOK__','20');
        
	elsif (tx1 in ('ConfirmacionBonoExt','AnulacionBonoExt') and length(soapaction1)=0 ) then
                soapaction1:='http://tempuri.org/wmImed_Srv'||split_part(tx1,'BonoExt',1);
                xml2:=put_campo(xml2,'__SECUENCIAOK__','20');

        elsif (tx1 in ('Confirmacion','Anulacion') and length(soapaction1)=0 ) then
                soapaction1:='http://tempuri.org/wmImed_Srv'||tx1;
                xml2:=put_campo(xml2,'__SECUENCIAOK__','20');
        else
                if length(soapaction1)=0 then
                        soapaction1:='http://tempuri.org/wmImed_Srv'||tx1;
                end if;
        ...
        --xml2:=logapp(xml2,'>>INPUT_CIA<< '||get_campo('INPUT_CIA',xml2));
...
begin
	...
	codigo_cia1:=get_campo('CODIGO_CIA',xml2);
        ...
        --Para el Bono3
        if (tipo_tx1 in ('ConfirmacionBono3') or tipo_tx1 in ('ConfirmacionBonoExt') ) then
				...
				--JCC 20150731: Incorpora un nuevo SP para BonoPapel 
				if tipo_tx1 in ('ConfirmacionBonoExt') then
                               		xml2:=put_campo(xml2,'SQLINPUT','call WsBcConfirmaSegVtaU('||extFolioBono1||','||extCodFinanciador1||','||get_campo('extFolioAuto',xml2)||')');
				else 
                               		xml2:=put_campo(xml2,'SQLINPUT','call CSConfirmaSegU('||extFolioBono1||','||extCodFinanciador1||','||get_campo('extFolioAuto',xml2)||','||codigo_cia1::integer||')');
				end if;
				...
				xml2:=put_campo(xml2,'__SECUENCIAOK__','90');
                                ...
                                xml2:=logapp(xml2,'Salgo del graba_tx_cia ');
                                return xml2;
                        --Si la cia me rechaza la confirmacion
                                ...
                                xml2:=logapp(xml2,'CF.B3.Graba Rechazo Cia '||tipo_tx1||' en Percona');
                                mensaje1:=replace(substring(get_campo('extMensajeError',xml2),1,30),chr(39),'<C>');
				
				--JCC 20150813: Incorpora un nuevo SP para BonoPapel
                                if tipo_tx1 in ('ConfirmacionBonoExt') then
                                	xml2:=put_campo(xml2,'SQLINPUT','call WsBcRechazoConfU('||extFolioBono1||','||extCodFinanciador1||','||quote_literal(mensaje1)||',1,'||quote_literal(extRutBenef1)||')');
                                else
        ...
        elsif (tipo_tx1 in ('AnulacionBono3') or tipo_tx1 in ('AnulacionBonoExt')) then
                xml2:=put_campo(xml2,'RESPUESTA_PERCONA','');
                xml2:=logapp(xml2,'B3-Respuesta Cia='||get_campo('extMensajeError',xml2)||' Status='||get_campo('extCodError',xml2));
                extFolioBono1:=get_xml('extFolioBono',input1);
                extCodFinanciador1:=get_xml('extCodFinanciador',input1);
                extRutBenef1:=get_xml('extRutBeneficiario',input1);
                --Verificamos generar SQLINPUT sin basura
				...
				--JCC 20150731: Incorpora un nuevo SP para BonoPapel 
				if tipo_tx1 in ('AnulacionBonoExt') then
                                	xml2:=put_campo(xml2,'SQLINPUT','call WsBcConfirmaSegAnuU('||extFolioBono1||','||quote_literal(extRutBenef1)||')');
				else 
                                	xml2:=put_campo(xml2,'SQLINPUT','call CSConfirmaAnuU('||extFolioBono1||','||quote_literal(extRutBenef1)||','||codigo_cia1::integer||')');
                                end if;
				...
				xml2:=put_campo(xml2,'__SECUENCIAOK__','90');
                        ...
                        --Si la cia me rechaza la confirmacion
                                ...
                                --Si Rechaza la Cia, Informa el Rechazo en Percona
                                xml2:=logapp(xml2,'ANU.b3.Graba Rechazo Cia '||tipo_tx1||' en Percona');
                                mensaje1:=replace(substring(get_campo('extMensajeError',xml2),1,30),chr(39),'<C>');
                                if (length(extRutBenef1)>0) then

					--JCC 20150731: Incorpora un nuevo SP para BonoPapel
	                                if tipo_tx1 in ('AnulacionBonoExt') then
                                        	xml2:=put_campo(xml2,'SQLINPUT','call WsBcRechazoConfU('||extFolioBono1||',0,'||quote_literal(mensaje1)||',2,'||quote_literal(extRutBenef1)||')');
                	                else
        ...
        --if (get_campo('extCodError',xml2)='N') then
                ...
                xml2:=logapp(xml2,'B2-Respuesta Cia='||get_campo('extMensajeError',xml2)||' Status='||get_campo('extCodError',xml2));
                --Si contesta bien la cia y el extFolioAuto es <> -1
                if (get_campo('extCodError',xml2)='S' and get_campo('extFolioAuto',xml2)<>'-1') then
