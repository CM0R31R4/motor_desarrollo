...
insert into isys_querys_tx values ('3003',100,1,2,'FONASA_WS',9000,110,112,1,1,30,30);   --Llama un servicio de tabla servicios - Apunta al servicio tradicional de FONASA.
insert into isys_querys_tx values ('3003',101,1,2,'FONASA_WS',9001,110,112,1,1,30,30);   --Llama un servicio de tabla servicios - Apunta a WebLogic de FONASA
...
declare
	...
	resultadoDiv1	integer; -- Variable usada para distinguir si va a Tuxedo o WebLogic.

	stFonasa        servicios_fonasa%ROWTYPE;
BEGIN
        xml2:=xml1;
	...
	xml2:=put_campo(xml2,'REQUEST_BONO', decode(get_campo('INPUT',xml2),'HEX')::varchar);
        ...
        if (cod_motor1 = '0') then
                xml2:=put_campo(xml2,'STATUS_HTTP','500 Falla Trx');
                xml2:=logapp(xml2,'CodMotor Error');
                --En caso de error, igual respondo un con un XML
                xml2:=put_campo(xml2,'RESPUESTA',stData.xml_error);
                --Proceso la respuesta con los datos en el XML
                xml2:=respuesta_xml(xml2);
...
/*	
	-- INICIO BALANCEO FONASA TUXEDO - WEBLOGIC
	
	-- Quita los dos ultimos digitos al codigo de motor ya que esos pertenecen a la maquina-
	cod_motor1:= substring(cod_motor1, 1, (length(cod_motor1) - 2));

	-- Se almacena el resultado.
	resultadoDiv1:=	cod_motor1::integer % 2;

	xml2:=logapp(xml2,'RESULTADO_DIVISION='||resultadoDiv1);
	xml2:=logapp(xml2,'CODIGO_MOTOR='||cod_motor1);

	-- Verifica que la transaccion entrante sea de FONASA.
	IF id_fin1 = 1 THEN
		IF (resultadoDiv1 = 0) THEN
			id_fin1:= 1;	-- Los pares se van a Tuxedo.
		ELSE
			id_fin1:= 15;	-- Los impares se van a WebLogic.
		END IF;
	END IF;
	-- FIN BALANCEO FONASA TUXEDO - WEBLOGIC
*/
	...
	/*--- Inicio WebLogic ---*/
        IF id_fin1 = 1  THEN
                SELECT * INTO stFonasa FROM servicios_fonasa WHERE tipo_tx=tipo_tx1 and financiador = id_fin1 and estado=1;
		...
		ELSE
        		xml2:=put_campo(xml2,'FUNCION_OUTPUT',stFonasa.funcion_output);
                	xml2:=put_campo(xml2,'FINANCIADOR',stFonasa.descripcion);

                	--EJECUTA FUNCION INPUT
                	xml2:=logapp(xml2,'Ejecuta='||stFonasa.funcion_input);
                	if length(stFonasa.funcion_input)>0 then
                        	EXECUTE 'SELECT ' || stFonasa.funcion_input || '(' || chr(39) || xml2 || chr(39) || ')' into xml2;
                	end if;

                	--Agrega los datos de lectura de la IP Generica
                	xml2:=put_campo(xml2,'DEFINE_SEC_WS_IP',stFonasa.ip_generica);
                	xml2:=put_campo(xml2,'DEFINE_SEC_WS_PORT',stFonasa.correlativo_port_generica);
                	xml2:=put_campo(xml2,'DEFINE_SEC_WS_SECUENCIAOK',stFonasa.secuencia::varchar);
			END IF;
	ELSE        
		-- ORIGINAL DEL FLUJO. NO BORRAR.
        	xml2:=put_campo(xml2,'FUNCION_OUTPUT',stSec.funcion_output);
        	xml2:=put_campo(xml2,'FINANCIADOR',stSec.descripcion);
	...
	*/
	...
	--IF get_campo('FINANCIADOR',xml2) = 'FONASA' THEN
		-- Servicios en dummy.
		-- Se deben comentar las dos lineas siguientes para apagar servicios dummy.
	--	xml2:=servicios_dummy_3003(xml2);
...
begin
	...
	-- [CM] - 11-12-2015 - Se agrega linea para mostrar datos en estadisticas de Sistema.
	xml2:=logapp(xml2,'run='||func_out1);
	-- Fin modificacion 11-12-2015.
	...
	EXECUTE 'SELECT ROUND(EXTRACT(MILLISECONDS FROM TIME '||chr(39)||timeout1::time||chr(39)||'))' INTO milliseconds1;
	...
	-- Muestra OK si el tiempo es menor a 10 segundos, sino es TimeOut.
        IF (milliseconds1::integer >= 10000) THEN
        	xml2:=logapp(xml2,func_out1||' returns=TIMEOUT elapsed_time='||milliseconds1||' ms');
       	ELSE
                xml2:=logapp(xml2,func_out1||' returns=OK elapsed_time='||milliseconds1||' ms');
        END IF;
		...
		-- Para estadisticas de Sistema envia 'uno' cuando hay ERROR.
                xml2:=logapp(xml2, 'API_Stat:3 Cod_Error:Conexion Fin ');
		...
		-- Para estadisticas de Sistema envia 'uno' cuando hay ERROR.
                xml2:=logapp(xml2, 'API_Stat:2 Cod_Error:Service ');
        ...
        if (strpos(get_campo('__STS_ERROR_SOCKET__',xml2),'FALLA_CONEXION')>0) then
                ...
                xml2:=put_campo(xml2,'MENSAJE_RESP','Falla Conexion con API');
                ...
                xml2:=logapp(xml2,'Falla:Conexion con API Tx='||tipo_tx1);
                if (tipo_tx1 in ('envbonis_real','envbono_real')) then
                        xml2:=put_campo(xml2,'STATUS_BONO','NK');
                        xml2:=sp_procesa_respuesta_cola_bonos(xml2);
                end if;
		...
		-- Para estadisticas de Sistema envia 'uno' cuando hay ERROR.
		xml2:=logapp(xml2, 'API_Stat:1 Cod_Error:Error API ');
	...
	-- Para estadisticas de Sistema envia 'cero' cuando esta todo OK.
      	xml2:=logapp(xml2,'API_Stat:0 Cod_Error:OK ');
	...
	-- Agrega respuesta en hexadecimal para campo en tabla tx_bono3 dinamica.
	xml2:=put_campo(xml2,'RESPONSE_BONO', decode(get_campo('RESPUESTA_HEX',xml2),'HEX')::varchar);
