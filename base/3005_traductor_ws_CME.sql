...
begin
		...
		--JCC 20150828 Si la tx es iniciotx, no va al secuencia SCRIPT
...
begin
        ...
        xml2:=logapp(xml2,'F-3005 Ejecuta_Funcion_Out_CME ='||func_out1);
        ...
        xml2:=logapp(xml2,'F-3005 Resp_FunOut="'||get_campo('MENSAJE_RESP',xml2)||'" Codigo='||get_campo('CODIGO_RESP',xml2));

        --Con este Tag envia el SpConcilacionCme por 2da Api. Mientras la 1ra se reinicia
        /*if (get_campo('LOOP_API',xml2)='SI') then
                xml2:=logapp(xml2,'F-3005_EnviaCme_Api2');
        ...
        xml2:=logapp(xml2,'F-3005_Timeout='||timeout1::varchar);
...
begin
        ...
        xml2:=logapp(xml2,'RESPUESTA_SYSTEM='||get_campo('RESPUESTA_SYSTEM',xml2));
