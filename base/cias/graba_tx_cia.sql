	...
	cod_fin1:=coalesce(nullif(get_campo('COD_FIN',xml2),''),'0')::integer;
        ...
        IF (tx1 in ('Certificacion','Conciliacion')) THEN
                ...
                EXCEPTION WHEN OTHERS THEN
                        EXECUTE 'create TABLE '||tx_tabla1||'(fecha_ingreso timestamp(6), dia integer DEFAULT(to_char(now(), ''YYYYMMDD''::text))::integer, tiempo_ini_cia timestamp(6), tiempo_fin_cia timestamp(6), tiempo_ini_sybase timestamp(6), tiempo_fin_sybase timestamp(6), codigo_cia varchar, codigo_motor bigint, extfolioauto varchar, extcoderror varchar, extmensajeerror varchar, msgoutput varchar, estado varchar, reintentos integer, tx varchar, nemo varchar, ip_cliente varchar, fecha_ult_modificacion timestamp(6), num_operacion varchar, extfoliobono varchar, cod_lugar varchar, rut_prestador varchar, rut_beneficiario varchar, msginput varchar, fecha_emision integer, monto_bonif integer, cod_fin integer, request_cia varchar, response_cia varchar);';
        ...
        ELSIF (tx1 in ('Confirmacion','Anulacion','ConfirmacionBono3','AnulacionBono3','ConfirmacionBonoExt','AnulacionBonoExt')) THEN
                ...
                BEGIN
						...
						cod_fin = '||cod_fin1||',
						request_cia = '||quote_literal(get_campo('REQUEST_CIA',xml2))||',
						response_cia = '||quote_literal(get_campo('RESPONSE_CIA',xml2))||'
