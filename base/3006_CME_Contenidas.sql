...
begin
	...
	xml2:=logapp(xml2,'Flujo_3006_In Contador=='||cont_cme1||' - TotalCme='||total_cme1);

	--Tomo el primer sql_CME. Envia a la Isapre.
	cod_motor_orig1	:=get_campo('COD_MOTOR_ORIG',xml2);
	tx_cme1		:=get_campo('TX_CME',xml2);
	sql_cme1	:=get_campo('SP_REQ',xml2);
	func_out1	:=get_campo('FUNC_SALIDA',xml2);
	...
	if (cont_cme1 <= total_cme1) then 
		xml2:=put_campo(xml2,'COD_MOTOR_CME',cod_motor_orig1);	--Se usa en /home/motor/base/CM/MASVIDA/cerenvcta_masvida.sql
		...
		xml2:=logapp(xml2,'Flujo_3006 Api.Persistencia = **'||cod_motor_orig1);
	else
		--Proceso todo, envia Commit y cierra la CME
                xml2:=put_campo(xml2,'SQLINPUT','SELECT 1 COMMIT TRANSACTION;');
		--xml2:=put_campo(xml2,'FUNC_OUT',func_out1);
		xml2:=put_campo(xml2,'__SECUENCIAOK__','88');

		xml2:=logapp(xml2,'Flujo_3006 FinalizaEnvio(COMMIT) IDXTX='||get_campo('COD_CME',xml2));
...
begin
	...
	if strpos(get_campo('SQLINPUT',xml2),'COMMIT') > 0 then
		...
		xml2:=logapp(xml2,'Glosa:CME idxTx='||get_campo('COD_CME',xml2)||'.Informadas');
		return xml2;
		...
		xml2:=logapp(xml2,'Glosa:CME idxTx='||get_campo('COD_CME',xml2)||'.Rollback'||'['||get_campo('CME_TX',xml2)||':'||get_campo('MSJ_FIN_CME',xml2)||']');
	...
	--Parseo respuesta con la funcion_output de cada CME.
		...
		--xml2:=logapp(xml2,'Flujo_3006 ExecFuncion_Out='||func_out1);
		...
		UPDATE tx_ctamed SET estado = get_campo('EST_CME',xml2), 
               	...
               	WHERE codigo_motor = cod_motor_orig1 and cod_ctamed = cod_cme1;
               	
		--Error en Update CME Procesada 
		if not found then
			xml2:=logapp(xml2,'CME Error Update tx_ctamed FunOut='||get_campo('FUNC_OUT',xml2)||' - CodMotorCME'||get_campo('COD_MOTOR_CME',xml2));
		...
		--Rollback solo debe venir de las funciones_outputs.
			...
			xml2:=logapp(xml2,'Viene Rollback. Cancela CME en el Financiador');						
	                xml2:=put_campo(xml2,'SQLINPUT','SELECT 1 ROLLBACK');
        	        xml2:=put_campo(xml2,'__SECUENCIAOK__','88');
			--Cierra Conexion Persistente, No Incrementa CONTADOR_CME
	                xml2:=put_campo(xml2,'__CONTROL_BD_PERSISTENTE__','CLOSE');
			xml2:=logapp(xml2,'Cierra Conexion __CONTROL_BD_PERSISTENTE__');						
			return xml2;
	        end if;

		--Debe volver con la sgte CME
		SELECT * into stCme from tx_ctamed where cod_fin=get_campo('ID_FIN',xml2)::integer and cod_ctamed=cod_cme1::integer and estado='CONTENIDA' and length(sp_exec)>0 order by fecha_in_tx asc limit 1;
		if found then 
                	xml2:=logapp(xml2,'fintx: Aun quedan Cuentas paara enviar tx_cme=% '||stCme.tipo_tx);
                	...
                	select * into stSec from define_secuencia_ws where tipo_tx=stCme.tipo_tx and financiador = get_campo('ID_FIN',xml2)::integer;
		...
		xml2:=logapp(xml2,'Vuelve a Secuencia=10 - Flujo=3006');
		...
		--xml2:=logapp(xml2,'Limpiando Tag SQLINPUT y FUNC_OUT');
		xml2:=put_campo(xml2,'CTACODERROR_1','');
                xml2:=put_campo(xml2,'CTAMENSAJEERROR_1','');
