delete from isys_querys_tx where llave='3001';

insert into isys_querys_tx values ('3001',10,1,1,'select proc_wsdl_3001(''$$__XMLCOMPLETO__$$'') as __XML__',0,0,0,1,1,-1,0);

--Parsea el WS para identificar la transacciones entrante
CREATE OR REPLACE FUNCTION proc_wsdl_3001(varchar)
returns varchar as
$$
declare
        xml1    	alias for $1;
	stWsdl		bono3.respuestas_soap%ROWTYPE;
        xml2    	varchar;
	tipo_tx1	varchar;
	input1		varchar;
	file_wsdl1	varchar;
...
begin
	...
	select * into stWsdl from bono3.respuestas_soap where strpos(input1,tipo_tx)>0;
	if not found then
		xml2:=put_campo(xml2,'STATUS_HTTP','400 Wsdl no encontrado');
		xml2:=logapp(xml2,'400 Wsdl no encontrado');
		xml2:=responde_http_get_3001(xml2);
		return xml2;
	end if;
	...
	if strpos(input1,'wsdl')>0 then
		--Leo el archivo WSDL
		--SELECT pg_read_file(stWsdl.file_wsdl) into file_wsdl1;
		...
		--Leo el archivo XSD
		--SELECT pg_read_file(stWsdl.file_xsd) into file_xsd1;
...
CREATE OR REPLACE FUNCTION responde_http_get_3001(varchar)
returns varchar as
$$
declare
        xml1            alias for $1;
...
begiN
        ...
        xml2:=put_campo(xml2,'RESPUESTA',encode(('HTTP/1.1 '||get_campo('STATUS_HTTP',xml2)||chr(10)||'Server: Apache-Coyote/1.1'||chr(10)||'Content-Type: text/xml'||chr(10)||'Content-Length: '||length(respuesta1)::varchar||chr(10)||'Date: '||to_char(now() AT TIME ZONE 'GMT','Dy, DD Mon YYYY HH24:MI:SS GMT')||chr(10)||chr(10)||respuesta1)::bytea,'hex'));
        return xml2;
end;
$$
LANGUAGE plpgsql;
