...
begin
	...
	xml2:=put_campo(xml2,'RESPONSE_BONO', get_campo('XML_OK',xml2));
	...
	-- Muestra OK si el tiempo es menor a 10 segundos, sino es TimeOut.
		...
		-- Para estadisticas de Sistema envia 'cero' cuando no hay conexion a API.
		xml2:=logapp(xml2, 'API_Stat:0 API_tx='||func_out1);

                return xml2;
	else
		-- Para estadisticas de Sistema envia 'uno' cuando si hay conexion a API.
		xml2:=logapp(xml2, 'API_Stat:1 API_tx='||func_out1);
        end if;
