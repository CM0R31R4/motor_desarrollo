CREATE OR REPLACE FUNCTION traductor_in_anulabonou_consalud(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	xml2:=put_campo(xml2,'SQLINPUT','["IMEDSOF.CONANULABONOU_PKG.CONANULABONOU", [ "$o$STRING", '||cod_fin1||','||chr(34)||folio_bono1||chr(34)||','||chr(34)||id_tratam1||chr(34)||','||chr(34)||fecha_tratam1||chr(34)||',"$o$STRING", "$o$STRING" ]]'||chr(10)||chr(10));
...
CREATE OR REPLACE FUNCTION traductor_out_anulabonou_consalud(varchar)
returns varchar as
$$
declare
        xml1		alias for $1;
...
begin
	...
	aux1:=json_field(resp1,'result');
        if aux1 is null then
                ...
                --Otro Error
                        ...
                        xml2:=put_campo(xml2,'ERRORCOD','99');
			xml2:=put_campo(xml2,'ERRORMSG','Consalud:Error_AnulaBonou');
                end if;
                xml2    :=put_campo(xml2,'CODIGO_RESP','2');
	...
	xml2:=logapp(xml2,'CONSALUD: RSP_ANULABONOU -> extCodResp1='||cod_resp1||' -extMensajeResp1='||mensaje_resp1);
