CREATE OR REPLACE FUNCTION traductor_in_infenrola_consalud(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	xml2:=put_campo(xml2,'SQLINPUT','["IMEDSOF.CONINFENROLA_PKG.CONINFENROLA", [ "$o$STRING", '||cod_fin1||','||chr(34)||rut_benef1||chr(34)||','||chr(34)||rut_acomp1||chr(34)||','||chr(34)||id_enrola1||chr(34)||',"$o$STRING", "$o$STRING" ]]'||chr(10)||chr(10));
...
CREATE OR REPLACE FUNCTION traductor_out_infenrola_consalud(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	resp1:=replace(get_campo('SQLOUTPUT',xml2),chr(10),'');
        ...
        if aux1 is null then
                ...
                --Otro Error
                        ...
                        xml2:=put_campo(xml2,'ERRORCOD','99');
			xml2:=put_campo(xml2,'ERRORMSG','Consalud:Error_InfEnrola');
                end if;
                xml2    :=put_campo(xml2,'CODIGO_RESP','2');
	...
	xml2:=logapp(xml2,'CONSALUD: RSP_INFENROLA -> extCodResp1='||cod_resp1||' -extMensajeResp1='||mensaje_resp1);
