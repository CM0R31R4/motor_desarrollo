CREATE OR REPLACE FUNCTION traductor_in_valtrans_vidacamara(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	xml2:=put_campo(xml2,'SQLINPUT',declare_params||' EXEC dbo.NMDValTrans '||ext_cod_financiador||','||ext_folio_financiador||',"'||ext_accion||'","'||ext_pregunta||'", '||output_params);
        return xml2;
...
CREATE OR REPLACE FUNCTION traductor_out_valtrans_vidacamara(varchar)
returns varchar as
$$
declare
        xml1            alias for $1;
...
begin
	...
	xml2:=logapp(xml2,'VIDACAMARA: RSP_VALTRANS -> extRespuesta1='||respuesta1||' -extCodResp1='||cod_resp1||' -extMensajeResp1='||mensaje_resp1);
