CREATE OR REPLACE FUNCTION traductor_in_anulabonou_vidacamara(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
xml2:=put_campo(xml2,'SQLINPUT',declare_params||' EXEC dbo.NMDAnulaBonoU '||ext_cod_financiador||','||ext_folio_bono||',"'||ext_ind_tratam||'","'||ext_fec_tratam||'",'||output_params);
        return xml2;
...
CREATE OR REPLACE FUNCTION traductor_out_anulabonou_vidacamara(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	xml2:=logapp(xml2,'VIDACAMARA: RSP_ANULABONOU -> extCodResp1='||cod_resp1||' -extMensajeResp1='||mensaje_resp1);
