CREATE OR REPLACE FUNCTION traductor_in_bencertif_vidacamara(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	exec_cmd:= 'EXEC dbo.NMDBenCertif '||cod_fin1||',"'||rut_benef1||'" ,"'||fecha_actual1||'", @extApellidoPat OUTPUT,@extApellidoMat OUTPUT,@extNombres OUTPUT, @extSexo OUTPUT,@extFechaNacimi OUTPUT,@extCodEstBen OUTPUT,@extDescEstado OUTPUT,@extRutCotizante OUTPUT, @extNomCotizante OUTPUT,@extDirPaciente OUTPUT,@extGlosaComuna OUTPUT,@extGlosaCiudad OUTPUT, @extPrevision OUTPUT,@extGlosa OUTPUT,@extPlan OUTPUT,@extDescuentoxPlanilla OUTPUT,@extMontoExcedente OUTPUT; ';
...
CREATE OR REPLACE FUNCTION traductor_out_bencertif_vidacamara(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	--Si vienen, los obtengo y aumento el contadoor
        while true loop
				...
				xml2:=logapp(xml2,'NMD_BENCERTIF_RECEIVE i='||i::varchar||' -rut_acompa1='||rut_acompa1||' -nom_acompa1='||nom_acompa1);
                        end if;
                        exit;
                end if;
                i:=i+1;
