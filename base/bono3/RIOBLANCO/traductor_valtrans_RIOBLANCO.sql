CREATE OR REPLACE FUNCTION traductor_in_valtrans_rioblanco(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	extCodFinanciador       := trim(get_campo('EXTCODFINANCIADOR',xml2));
        ...
        xml2:=put_campo(xml2,'SQLINPUT','["ADMIMED.RBLVALTRANS_PKG.RBLVALTRANS", ["$o$STRING", '||extCodFinanciador||', '||exFolioFinanciador||', '||chr(34)||extAccion||chr(34)||', '||chr(34)||extPregunta||chr(34)||', "$o$STRING", "$o$STRING", "$o$STRING"]]'||chr(10)||chr(10));
...
CREATE OR REPLACE FUNCTION traductor_out_valtrans_rioblanco(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	xml2    :=put_campo(xml2,'ESTADO_TX','TERMINADO_OK');
        ...
        if aux1 is null then
                ...
                if aux1 is not null then
                        ...
                        xml2:=put_campo(xml2,'ERRORCOD','99');
			xml2:=put_campo(xml2,'ERRORMSG','RioBlanco:Error_Valtrans');
                end if;
                xml2    :=put_campo(xml2,'CODIGO_RESP','2');
	...
	xml2:=logapp(xml2,'RIOBLANCO: RSP_VALTRANS -> extRespuesta1='||respuesta1||' -extCodResp1='||cod_resp1||' -extMensajeResp1='||mensaje_resp1);
