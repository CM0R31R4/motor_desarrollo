CREATE OR REPLACE FUNCTION traductor_in_valorvari_rioblanco(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	cod_fin1       := trim(get_campo('EXTCODFINANCIADOR',xml2));
        ...
        rut_conv1          	:= get_campo('EXTRUTBENEFICIARIO',xml2);
...
CREATE OR REPLACE FUNCTION traductor_out_valorvari_rioblanco(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
	...
	xml2    :=put_campo(xml2,'ESTADO_TX','TERMINADO_OK');
        ...
        if aux1 is null then
                ...
                if aux1 is not null then
                        ...
                        xml2:=put_campo(xml2,'ERRORCOD','99');
			xml2:=put_campo(xml2,'ERRORMSG','RioBlanco:Error_ValorVari');
                end if;
                xml2    :=put_campo(xml2,'CODIGO_RESP','2');
