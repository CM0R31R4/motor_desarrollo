CREATE OR REPLACE FUNCTION traductor_in_valtrans_sanlorenzo(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	xml2:=put_campo(xml2,'SQLINPUT','["ADMIMED.SNLVALTRANS_PKG.SNLVALTRANS", [ "$o$STRING", '||cod_fin1||','||chr(34)||folio_fin1||chr(34)||','||chr(34)||accion1||chr(34)||','||chr(34)||pregunta1||chr(34)||', "$o$STRING","$o$STRING","$o$STRING" ]]'||chr(10)||chr(10));
...
CREATE OR REPLACE FUNCTION traductor_out_valtrans_sanlorenzo(varchar)
returns varchar as
$$
declare
        xml1		alias for $1;
...
begin
	...
	resp1:=replace(get_campo('SQLOUTPUT',xml2),chr(10),'');
        ...
        if aux1 is null then
                ...
                --Otro Error
                        ...
                        xml2:=put_campo(xml2,'ERRORCOD','99');
			xml2:=put_campo(xml2,'ERRORMSG','SanLorenzo:Error_Valtrans');
                end if;
                xml2    :=put_campo(xml2,'CODIGO_RESP','2');
	...
	xml2:=logapp(xml2,'SANLORENZO: RSP_VALTRANS -> extRespuesta1='||respuesta1||' -extCodResp1='||cod_resp1||' -extMensajeResp1='||mensaje_resp1);
