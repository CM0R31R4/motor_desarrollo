CREATE OR REPLACE FUNCTION traductor_in_anulabonou_banmedica(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	xml2:=put_campo(xml2,'SQLINPUT','["INFOMEDICA.BANANULABONOU_PKG.BANANULABONOU", [ "$o$STRING", '||cod_fin1||','||chr(34)||folio_bono1||chr(34)||','||chr(34)||id_tratam1||chr(34)||','||chr(34)||fecha_tratam1||chr(34)||',"$o$STRING", "$o$STRING" ]]'||chr(10)||chr(10));
...
CREATE OR REPLACE FUNCTION traductor_out_anulabonou_banmedica(varchar)
returns varchar as
$$
declare
        xml1		alias for $1;
...
begin
        ...
        if aux1 is null then
			...
			--xml2:=put_campo(xml2,'ERRORMSG',replace(aux1,'\n',' '));
                --Otro Error
                        ...
                        xml2:=put_campo(xml2,'ERRORCOD','99');
                        xml2:=put_campo(xml2,'ERRORMSG','Banmedica:Error_AnulaBonou');
                end if;
		xml2    :=put_campo(xml2,'CODIGO_RESP','2');
	...
	xml2:=logapp(xml2,'BANMEDICA: RSP_ANULABONOU -> extCodError='||cod_resp1||' -extMensajeError='||mensaje_resp1);
