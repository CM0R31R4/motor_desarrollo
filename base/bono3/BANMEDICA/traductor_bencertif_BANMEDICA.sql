CREATE OR REPLACE FUNCTION traductor_in_bencertif_banmedica(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	cod_fin1        :=trim(get_campo('EXTCODFINANCIADOR',xml2));
        ...
        fecha_actual1   :=to_char(get_campo('EXTFECHAACTUAL',xml2)::date, 'DD-MON-YYYY');

--fecha_aux1:=to_char((input1)::date , 'DD-MON-YYYY');  --23-JUL-2013
--select to_char(to_timestamp(to_char(07, '999'), 'MM'), 'MON')
--select to_char(('2013-07-23')::date , 'DD-MON-YYYY');
--fecha_actual1='14-JUN-13'; --Formato OK. Puede ir sin guiones
	...
	--El RUT debe viajar sin el digito y guion. Ya que Banmedica castea el varchar a integer.
	--"ORA-06502: PL/SQL: numeric or value error: character to number conversion error"
	--rut_ben1:=split_part(rut_ben1,'-',1);
        xml2    :=put_campo(xml2,'RUT_BASE',rut_ben1);
	
	--xml2:=put_campo(xml2,'INPUT','["INFOMEDICA.BANBENCERTIF_PKG.BANBENCERTIF", [ "$o$STRING", '||cod_fin1||','||chr(34)||rut_ben1||chr(34)||','||chr(34)||fecha_actual1||chr(34)||', "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$NUMBER", "$o$STRING['||l_array||']", "$o$STRING['||l_array||']" ]]'||chr(10)||chr(10));
	xml2:=put_campo(xml2,'SQLINPUT','["INFOMEDICA.BANBENCERTIF_PKG.BANBENCERTIF", [ "$o$STRING", '||cod_fin1||','||chr(34)||rut_ben1||chr(34)||','||chr(34)||fecha_actual1||chr(34)||', "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$NUMBER", "$o$STRING['||l_array||']", "$o$STRING['||l_array||']" ]]'||chr(10)||chr(10));
...
CREATE OR REPLACE FUNCTION traductor_out_bencertif_banmedica(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	--Si no esta el tag result es porque hay error
        ...
        if aux1 is null then
                ...
                --Otro Error
                        ...
                        xml2:=put_campo(xml2,'ERRORCOD','99');
                        xml2:=put_campo(xml2,'ERRORMSG','Banmedica:Error_Bencertif');
                end if;
		xml2    :=put_campo(xml2,'CODIGO_RESP','2');
	...
	xml2:=logapp(xml2,'Result Aux1='||aux1);
		
	apell_pat1	:=limpia_caracteres_web(trim(replace(json_field(aux1,'1'),'"','')));
	apell_mat1	:=limpia_caracteres_web(trim(replace(json_field(aux1,'2'),'"','')));
	nombre1		:=limpia_caracteres_web(trim(replace(json_field(aux1,'3'),'"','')));
	...
	nom_cotiza1	:=limpia_caracteres_web(trim(replace(json_field(aux1,'9'),'"','')));
