CREATE OR REPLACE FUNCTION traductor_in_benrec_banmedica(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	xml2:=put_campo(xml2,'SQLINPUT','["INFOMEDICA.BANBENREC_PKG.BANBENREC", [ "$o$STRING", '||cod_fin1||','||chr(34)||rut_cotiza1||chr(34)||','||chr(34)||corr_benef1||chr(34)||','||chr(34)||rut_ben1||chr(34)||',"$o$STRING", "$o$STRING" ]]'||chr(10)||chr(10));
...
CREATE OR REPLACE FUNCTION traductor_out_benrec_banmedica(varchar)
returns varchar as
$$
declare
        xml1		alias for $1;
...
begin
	...
	resp1:=replace(get_campo('SQLOUTPUT',xml2),chr(10),'');
        ...
        if aux1 is null then
                ...
                --Otro Error
                        ...
                        xml2:=put_campo(xml2,'ERRORCOD','99');
			xml2:=put_campo(xml2,'ERRORMSG','Banmedica:Error_Benrec');
                end if;
                xml2    :=put_campo(xml2,'CODIGO_RESP','2');
	...
	xml2:=logapp(xml2,'BANMEDICA: RSP_BENREC -> extCodResBen='||cod_resp1||' -extMensajeError='||mensaje_resp1);
