CREATE OR REPLACE FUNCTION traductor_in_valorizi_masvida(varchar)
returns varchar as
$$
declare
...
CREATE OR REPLACE FUNCTION traductor_out_valorizi_masvida(varchar)
returns varchar as
$$
declare
	xml1  	alias for $1;
...
begin
	...
	while true loop
		valor_aux1      :=replace(trim(get_campo('@EXTVALORPRESTACION_'||i::varchar,xml2)),'@','');
        	aporte_aux1     :=replace(trim(get_campo('@EXTAPORTEFINANCIADOR_'||i::varchar,xml2)),'@','');
	        copago_aux1     :=replace(trim(get_campo('@EXTCOPAGO_'||i::varchar,xml2)),'@','');
	        interno_aux1    :=replace(trim(get_campo('@EXTINTERNOISA_'||i::varchar,xml2)),'@','');
	...
	xml2:=logapp(xml2,'MASVIDA: RSP_VALORIZI -> extCodResp1='||cod_resp1||' -extMensajeResp1='||mensaje_resp1||' -extPlanResp1='||plan_resp1||' -extGlosa1='||glosa1||' -extGlosa2='||glosa2||' -extGlosa3='||glosa3||' -extGlosa4='||glosa4||' - extGlosa5='||glosa5||' -extValorPrest1='||valor_prest1||' -extAporteFin1='||aporte_fin1||' -extCopago1='||copago1||' -extInternoIsa1='||interno_isa1||' -extTipoBonif1='||tipo_bonif1||' -extCopago1='||ext_copago1);
