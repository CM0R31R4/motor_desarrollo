CREATE OR REPLACE FUNCTION traductor_in_bencertif_cruzdelnorte(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	xml2:=put_campo(xml2,'SQLINPUT','["ISAPRE_ADM.CDNBENCERTIF_PKG.CDNBENCERTIF", [ "$o$STRING", '||cod_fin1||','||chr(34)||rut_benef1||chr(34)||','||chr(34)||fecha_actual1||chr(34)||', "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$NUMBER", "$o$STRING['||l_array||']", "$o$STRING['||l_array||']" ]]'||chr(10)||chr(10));
...
CREATE OR REPLACE FUNCTION traductor_out_bencertif_cruzdelnorte(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	--Si no esta el tag result es porque hay error
        ...
        if aux1 is null then
                ...
                --Otro Error
                        ...
                        xml2:=put_campo(xml2,'ERRORCOD','99');
			xml2:=put_campo(xml2,'ERRORMSG','CruzDelNorte:Error_Bencertif');
                end if;
                xml2    :=put_campo(xml2,'CODIGO_RESP','2');
	...
	xml2:=logapp(xml2,'CRUZDELNORTE: RSP_BENCERTIF -> extApellPat1='||apell_pat1||' -extApellMat1='||apell_mat1||' -extNombre1='||nombre1||' -extSexo1='||sexo1||' -extFechaNac1='||fecha_nac1||' -extCodEstBen1='||cod_est_ben1||' -extDescEstado1='||desc_estado1||' -extRutCotiza1='||rut_cotiza1||' -extNomCotiza1='||nom_cotiza1||' -extDireccion1='||direccion1||' -extGlosaComuna1='||glosa_comuna1||' -extGlosaCiudad1='||glosa_ciudad1||' -extPrevision1='||prevision1||' -extGlosa1='||glosa1||' -extPlan1='||plan1||' -extDsctoPlanilla1='||dscto_planilla1||' -extMontoExec1='||monto_exec1||' -extRutAcompa1='||rut_acompa1||' -extNomAcompa1='||nom_acompa1);

	--xml2:=logapp(xml2,'CDN_BENCERTIF_RECEIVE extApellidoPat='||apell_pat1||' -extApellidoMat='||apell_mat1||' -extNombres='||nombre1||' -extSexo='||sexo1||' -extFechaNacimi='||fecha_nac1||' -extCodEstBen='||cod_est_ben1);
