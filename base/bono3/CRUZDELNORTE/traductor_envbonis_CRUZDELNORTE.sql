CREATE OR REPLACE FUNCTION traductor_in_envbonis_cruzdelnorte(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	num_conv1       :=get_campo('EXTHOMNUMEROCONVENIO',xml2);
        ...
        monto_exec1     :=get_campo('EXTMONTOEXCEDENTE',xml2);    -- numeric(10),
                        --replace(get_campo('EXTFECHAEMISION',xml2),'-','');
        ...
        tipo_solic1     :=get_campo('EXTTIPOSOLICITUD',xml2);     -- numeric(10),
                        --replace(get_campo('EXTFECHAEMISION',xml2),'-','');
        fecha_inicio1   :=to_char(get_campo('EXTFECHAEMISION',xml2)::date, 'YYYYMMDD');
	...
	xml2:=put_campo(xml2,'SQLINPUT','["ISAPRE_ADM.CDNENVBONIS_PKG.CDNENVBONIS", [ "$o$STRING", '||cod_fin1||','||chr(34)||num_conv1||chr(34)||','||chr(34)||lugar_conv1||chr(34)||','||chr(34)||suc_venta1||chr(34)||','||chr(34)||rut_conv1||chr(34)||','||chr(34)||rut_asoc1||chr(34)||','||chr(34)||nom_prest1||chr(34)||','||chr(34)||rut_tratante1||chr(34)||','||chr(34)||especialidad1||chr(34)||','||chr(34)||rut_benef1||chr(34)||','||chr(34)||rut_cotiza1||chr(34)||','||chr(34)||rut_acomp1||chr(34)||','||chr(34)||rut_emisor1||chr(34)||','||chr(34)||rut_cajero1||chr(34)||','||chr(34)||cod_diag1||chr(34)||','||chr(34)||dscto_planilla1||chr(34)||','||monto_exec1||','||chr(34)||fecha_emision1||chr(34)||','||nivel_conv1||','||folio_financ1||','||monto_valor1||','||monto_aporte1||','||monto_copago1||','||num_opera1||','||corr_presta1||','||tipo_solic1||','||chr(34)||fecha_inicio1||chr(34)||','||chr(34)||urgencia1||chr(34)||','||chr(34)||plan1||chr(34)||','||chr(34)||lista1||chr(34)||','||chr(34)||lista2||chr(34)||','||chr(34)||lista3||chr(34)||', "$o$STRING", "$o$STRING"]]'||chr(10)||chr(10));
...
CREATE OR REPLACE FUNCTION traductor_out_envbonis_cruzdelnorte(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	resp1:=replace(get_campo('SQLOUTPUT',xml2),chr(10),'');
        ...
        if aux1 is null then
                ...
                --Otro Error
                        ...
                        xml2:=put_campo(xml2,'ERRORCOD','99');
			xml2:=put_campo(xml2,'ERRORMSG','CruzDelNorte:Error_EnvBonIs');
                end if;
                xml2    :=put_campo(xml2,'CODIGO_RESP','2');
	...
	xml2:=logapp(xml2,'CRUZDELNORTE: RSP_ENVBONIS -> extCodResp1='||cod_resp1||' -extMensajeResp1='||mensaje_resp1);
