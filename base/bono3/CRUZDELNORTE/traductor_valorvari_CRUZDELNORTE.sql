CREATE OR REPLACE FUNCTION traductor_in_valorvari_cruzdelnorte(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	xml2:=put_campo(xml2,'SQLINPUT','["ISAPRE_ADM.CDNVALORVARI_PKG.CDNVALORVARI", [ "$o$STRING", '||cod_fin1||','||chr(34)||num_conv1||chr(34)||','||chr(34)||lugar_conv1||chr(34)||','||chr(34)||suc_venta1||chr(34)||','||chr(34)||rut_conv1||chr(34)||','||chr(34)||rut_tratante1||chr(34)||','||chr(34)||especialidad1||chr(34)||','||chr(34)||rut_solic1||chr(34)||','||chr(34)||rut_benef1||chr(34)||','||chr(34)||tratamiento1||chr(34)||','||chr(34)||cod_diag1||chr(34)||','||nivel_conv1||','||chr(34)||urgencia1||chr(34)||','||chr(34)||lista1||chr(34)||','||chr(34)||lista2||chr(34)||','||chr(34)||lista3||chr(34)||','||chr(34)||lista4||chr(34)||','||chr(34)||lista5||chr(34)||','||chr(34)||lista6||chr(34)||','||chr(34)||lista7||chr(34)||','||num_prestad1||',"$o$STRING","$o$STRING","$o$STRING","$o$STRING","$o$STRING","$o$STRING","$o$STRING","$o$STRING","$o$NUMBER['||l_array||']","$o$NUMBER['||l_array||']","$o$NUMBER['||l_array||']","$o$STRING['||l_array||']","$o$NUMBER['||l_array||']","$o$NUMBER['||l_array||']","$o$NUMBER['||l_array||']","$o$NUMBER['||l_array||']","$o$NUMBER['||l_array||']","$o$NUMBER['||l_array||']","$o$NUMBER['||l_array||']","$o$NUMBER['||l_array||']","$o$NUMBER['||l_array||']","$o$NUMBER['||l_array||']" ]]'||chr(10)||chr(10));
...
CREATE OR REPLACE FUNCTION traductor_out_valorvari_cruzdelnorte(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	resp1:=replace(get_campo('SQLOUTPUT',xml2),chr(10),'');
        ...
        if aux1 is null then
                ...
                --Otro Error
                        ...
                        xml2:=put_campo(xml2,'ERRORCOD','99');
			xml2:=put_campo(xml2,'ERRORMSG','CruzDelNorte:Error_Valorvari');
                end if;
                xml2    :=put_campo(xml2,'CODIGO_RESP','2');
	...
	xml2:=logapp(xml2,'CRUZDELNORTE: RSP_VALORVARI -> extCodResp1='||cod_resp1||' -extMensajeResp1='||mensaje_resp1||' -extPlanResp1='||plan_resp1||' -extGlosa1='||glosa1||' -extGlosa2='||glosa2||' -extGlosa3='||glosa3||' -extGlosa4='||glosa4||' - extGlosa5='||glosa5||' -extValorPrest1='||valor_prest1||' -extAporteFin1='||aporte_fin1||' -extCopago1='||copago1||' -extInternoIsa1='||interno_isa1||' -extTipoBonif1='||tipo_bonif1||' -extCopago1='||ext_copago1||' -extTipoBonif2='||tipo_bonif2||' -extCopago2='||extCopago2||' -extTipoBonif3='||tipo_bonif3||' -extCopago3='||ext_copago3||' -extTipoBonif4='||tipo_bonif4||' -extCopago4='||ext_copago4||' -extTipoBonif5='||tipo_bonif5||' -extCopago5='||ext_copago5);
