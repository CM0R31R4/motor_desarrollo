CREATE OR REPLACE FUNCTION bono3.traductor_in_infenrola_vidatres(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	extCodFinanciador       := trim(get_campo('EXTCODFINANCIADOR',xml2));
        ...
        extIndEnrola            := coalesce(nullif(get_campo('EXTINDENROLA',xml2),''),'0');
	...
	if length(rut_benef1)>'0' then rut_benef1       :=lpad(rut_benef1,10,'0')||'-'||motor_modulo11(rut_benef1);end if;
        ...
        xml2:=put_campo(xml2,'INPUT','["INFOMEDICA.VIDINFENROLA_PKG.VIDINFENROLA", [ "$o$STRING", '||extCodFinanciador||', '||chr(34)||rut_benef1||chr(34)||', '||chr(34)||rut_acomp1||chr(34)||', '||chr(34)||extIndEnrola||chr(34)||', "$o$STRING", "$o$STRING" ]]'||chr(10)||chr(10));
...
CREATE OR REPLACE FUNCTION bono3.traductor_out_infenrola_vidatres(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
