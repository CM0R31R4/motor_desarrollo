CREATE OR REPLACE FUNCTION bono3.traductor_in_valtrans_vidatres(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	extCodFinanciador       := trim(get_campo('EXTCODFINANCIADOR',xml2));
        ...
        xml2:=put_campo(xml2,'INPUT','["INFOMEDICA.VIDVALTRANS_PKG.VIDVALTRANS", ["$o$STRING", '||extCodFinanciador||', '||exFolioFinanciador||', '||chr(34)||extAccion||chr(34)||', '||chr(34)||extPregunta||chr(34)||', "$o$STRING", "$o$STRING", "$o$STRING"]]'||chr(10)||chr(10));
...
CREATE OR REPLACE FUNCTION bono3.traductor_out_valtrans_vidatres(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
