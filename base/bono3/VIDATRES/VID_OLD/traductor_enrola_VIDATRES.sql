CREATE OR REPLACE FUNCTION bono3.traductor_in_enrola_vidatres(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	if length(rut_enrolar1)>'0' then rut_enrolar1     :=lpad(rut_enrolar1,10,'0')||'-'||motor_modulo11(rut_enrolar1);end if;
        ...
        xml2:=put_campo(xml2,'INPUT','["INFOMEDICA.VIDENROLA_PKG.VIDENROLA", [ "$o$STRING", '||extCodFinanciador||', '||chr(34)||rut_enrolar1||chr(34)||', '||chr(34)||rut_benef1||chr(34)||', "$o$STRING", "$o$STRING" ]]'||chr(10)||chr(10));
...
CREATE OR REPLACE FUNCTION bono3.traductor_out_enrola_vidatres(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
