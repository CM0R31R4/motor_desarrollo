CREATE OR REPLACE FUNCTION traductor_in_cermensajeben_vidatres(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	rut_ben1        :=get_campo('EXTRUTBENEFICIARIO',xml2);
        ...
        xml2:=put_campo(xml2,'SQLINPUT','["INFOMEDICA.VIDMENSAJEBEN_PKG.VIDMENSAJEBEN_PKG", [ "$o$STRING", '||cod_fin1||', '||chr(34)||rut_ben1||chr(34)||', "$o$NUMBER", "$o$STRING","$o$STRING" ]]'||chr(10)||chr(10));
...
CREATE OR REPLACE FUNCTION traductor_out_cermensajeben_vidatres(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	resp1	=replace(get_campo('SQLOUTPUT',xml2),chr(10),'');
        ...
        if aux1 is null then
                ...
                --Otro Error
                        ...
                        xml2:=put_campo(xml2,'ERRORCOD','99');
			xml2:=put_campo(xml2,'ERRORMSG','VidaTres:Error_CerMensajeBen');
                end if;
                xml2    :=put_campo(xml2,'CODIGO_RESP','2');
	...
	xml2:=logapp(xml2,'VIDATRES: RSP_CERMENSAJEBEN -> extTipoAccion1='||tipo_accion1||' -extMsgError1='||msg_error1||' -extMsgError2='||msg_error2);

	--raise notice 'JCC_MENSAJEBEN_Receive tipo_accion1=% - msg_error1=% - msg_error2=% ',tipo_accion1,msg_error1,msg_error2;
        ...
        xml2:=put_campo(xml2,'EXTMSGERROR1',msg_error2);
