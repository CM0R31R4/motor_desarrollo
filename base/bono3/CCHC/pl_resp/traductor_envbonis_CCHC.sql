CREATE OR REPLACE FUNCTION bono3.traductor_in_envbonis_cchc(varchar)
returns varchar as
...
begin
	...
	xml2    :=put_campo(xml2,'_SECUENCIA_','0');
        ...
        ext_hom_numero_convenio :=get_campo('EXTHOMNUMEROCONVENIO',xml2);	--Acepta el -
        ...
        ext_nom_prestador 	:=get_campo('EXTNOMPRESTADOR',xml2);
        ...
        ext_folio_financiador	:=get_campo('EXTFOLIOFINANCIADOR',xml2); --20
        ...
        ext_lista_1 		:=get_campo('EXTLISTA1',xml2); --30
	...
	--Valida formato del Rut
        ...
        declare_params:=' DECLARE @extCodigoError VARCHAR(2)
                          DECLARE @extMensajeError VARCHAR(100)';

        out_params:= ' @extCodigoError OUTPUT,@extMensajeError OUTPUT; select @extCodigoError as extCodigoError, @extMensajeError as extMensajeError; ';
...
CREATE OR REPLACE FUNCTION bono3.traductor_out_envbonis_cchc(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
