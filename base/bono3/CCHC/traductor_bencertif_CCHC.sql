CREATE OR REPLACE FUNCTION traductor_in_bencertif_cchc(varchar)
returns varchar as
$$
declare
        xml1    	alias for $1;
...
CREATE OR REPLACE FUNCTION traductor_out_bencertif_cchc(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	while true loop
			...
			--Quito la ultima coma (ya que no volvera al ciclo) y se sale
				...
				xml2:=logapp(xml2,'SMD_BENCERTIF_RECEIVE i='||i::varchar||' -rut_acompa1='||rut_acompa1||' -nom_acompa1='||nom_acompa1);
			end if;
                        exit;
                end if;
                i:=i+1;
