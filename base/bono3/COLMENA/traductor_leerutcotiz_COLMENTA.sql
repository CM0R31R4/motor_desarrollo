CREATE OR REPLACE FUNCTION traductor_in_leerutcotiz_colmena(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	declare_params:='DECLARE @extCodFinanciador numeric (5)
	        ...
	        DECLAR @extNomCotizante char(40)
	...
	exec_sp:= 'execute CGCLeeRutCotiz '||cod_fin1||',"'||rut_cotiz1||'", @extNomCotizante OUTPUT, @extCodError OUTPUT, @extMensajeError OUTPUT ';
	/*Ejemplo*/ 
	--exec_sp:= exec CGCLeeRutCotiz 78, '0005308213-0', @extNomCotizante output, @extCodError output, @extMensajeError output;
...
CREATE OR REPLACE FUNCTION traductor_out_leerutcotiz_colmena(varchar)
returns varchar as
$$
declare
        xml1		alias for $1;
...
begin
	...
	xml2:=logapp(xml2,'COLMENA: RSP_LEERUTCOTIZ -> extNombreCotiz1='||nombre_cotiz1||'  -extCodResp1='||cod_resp1||' -extMensajeResp1='||mensaje_resp1||' -extCorrBenef1='||corr_benef1||' -extRutBenef1='||rut_benef1||' -extApellPat1='||apell_pat1||' -extApellMat1='||apell_mat1||' -extNombres1='||nombres1||' -extCodEstBenef1='||cod_est_benef1||' -extDescEstado1='||desc_estado1);
