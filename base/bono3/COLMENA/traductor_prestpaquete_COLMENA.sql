CREATE OR REPLACE FUNCTION traductor_in_prestpaquete_colmena(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	exec_sp:= 'execute isapre..CGCPrestPaquete '||cod_fin1||',"'||num_conv1||'","'||lugar_conv1||'","'||cod_paquete1||'", @extCodError OUTPUT, @extMensajeError OUTPUT ';
	/*Ejemplo*/ 
	--exec_sp:= exec isapre..CGCPrestPaquete 078, '00000042095-000', '0070681', '819308', @extCodError output, @extMensajeError output;
...
CREATE OR REPLACE FUNCTION traductor_out_prestpaquete_colmena(varchar)
returns varchar as
$$
declare
        xml1		alias for $1;
...
begin
	...
	while true loop
		...
		raise notice 'JCC_1';
			...
			cod_homo1     	:=cod_homo1||cod_aux1||'"'||','||'"';
                        ...
                        cantidad1      	:=apell_pat1||cant_aux1||',';
			raise notice 'JCC_2';
		else 
	...
	xml2:=logapp(xml2,'COLMENA: RSP_PRESTPAQUETE -> extCodResp1='||cod_resp1||' -extMensajeResp1='||mensaje_resp1||' -extCodHomo1='||cod_homo1||' -extItemHomol1='||item_homo1||' -extCantidad1='||cantidad1);
