CREATE OR REPLACE FUNCTION traductor_in_valorvari_colmena(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	exec_sp:= 'execute isapre..CGCValorVari '||cod_fin1||',"'||num_conv1||'","'||lugar_conv1||'","'||suc_venta1||'","'||rut_conv1||'","'||rut_tratante1||'","'||especialidad1||'", "'||rut_solic1||'","'||rut_benef1||'","'||tratamiento1||'","'||cod_diag1||'",'||nivel_conv1||',"'||urgencia1||'","'||lista1||'","'||lista2||'","'||lista3||'","'||lista4||'","'||lista5||'","'||lista6||'","'||lista7||'",'||num_prestad1||', @extCodError output, @extMensajeError output, @extPlan output,@extGlosa1 output, @extGlosa2 output, @extGlosa3 output, @extGlosa4 output, @extGlosa5 output ';
		
	--exec_sp:= 'execute isapre..CGCValorVari '||cod_fin1||',['||num_conv1||'],['||lugar_conv1||'],['||suc_venta1||'],['||rut_conv1||'],['||rut_tratante1||'],['||especialidad1||'], ['||rut_solic1||'],['||rut_benef1||'],['||tratamiento1||'],['||cod_diag1||'],'||nivel_conv1||',['||urgencia1||'],"'||lista1||'","'||lista2||'","'||lista3||'","'||lista4||'","'||lista5||'","'||lista6||'","'||lista7||'",'||num_prestad1||', @extCodError output, @extMensajeError output, @extPlan output,@extGlosa1 output, @extGlosa2 output, @extGlosa3 output, @extGlosa4 output, @extGlosa5 output ';


	/*Ejemplo SP_SYBASE*/
	-- exec isapre..CGCValorVari 78, '52119-0', '83825','130600', '76016305-8','0076016305-8', '', '0012259049-6','0013486200-9', 'N','XXXXXXXXXX',0,'N','0405001000|  |0405001000     |N|01|000000000000|','','','','','','',1,@extCodError output, @extMensajeError output, @extPlan output,@extGlosa1 output, @extGlosa2 output,  @extGlosa3 output, @extGlosa4 output, @extGlosa5 output;
...
CREATE OR REPLACE FUNCTION traductor_out_valorvari_colmena(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	xml2:=logapp(xml2,'COLMENA: RSP_VALORVARI -> extCodResp1='||cod_resp1||' -extMensajeResp1='||mensaje_resp1||' -extPlanResp1='||plan_resp1||' -extGlosa1='||glosa1||' -extGlosa2='||glosa2||' -extGlosa3='||glosa3||' -extGlosa4='||glosa4||' - extGlosa5='||glosa5||' -extValorPrest1='||valor_prest1||' -extAporteFin1='||aporte_fin1||' -extCopago1='||copago1||' -extInternoIsa1='||interno_isa1||' -extTipoBonif1='||tipo_bonif1||' -extCopago1='||ext_copago1);
