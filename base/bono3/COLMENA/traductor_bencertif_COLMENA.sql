CREATE OR REPLACE FUNCTION traductor_in_bencertif_colmena(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	--Fecha Actual, debe ser hoy si o si
	...
	/*Ejemplo*/
	--exec isapre..CGCBenCertif 078, '001227910-8', '20101124', @extApellidoPat output, @extApellidoMat output, @extNombres output, @extSexo output, @extFechaNacimi output, @extCodEstBen output, @extDescEstado output, @extRutCotizante output, @extNomCotizante output, @extDirPaciente output, @extGlosaComuna output, @extGlosaCiudad output, @extPrevision output, @extGlosa output, @extPlan output, @extDescuentoxPlanilla output, @extMontoExcedente output;
...
CREATE OR REPLACE FUNCTION traductor_out_bencertif_colmena(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
        ...
        while true loop
		 ...
		 if (length(rut_aux1)>0 or length(nom_aux1)>0) and (rut_aux1<>'NULL' and nom_aux1<>'NULL') then
			...
			--Quito la ultima coma (ya que no volvera al ciclo) y se sale
				...
				xml2:=logapp(xml2,'CGC_BENCERTIF_RECEIVE i='||i::varchar||' -rut_acompa1='||rut_acompa1||' -nom_acompa1='||nom_acompa1);
                        end if;	
                        exit;
                end if;
                i:=i+1;
	...
	rut_acompa1	:=limpia_caracteres_web(rut_acompa1);
	nom_acompa1	:=limpia_caracteres_web(nom_acompa1);
	...
	xml2:=logapp(xml2,'COLMENA: RSP_BENCERTIF -> extApellPat1='||apell_pat1||' -extApellMat1='||apell_mat1||' -extNombre1='||nombre1||' -extSexo1='||sexo1||' -extFechaNac1='||fecha_nac1||' -extCodEstBen1='||cod_est_ben1||' -extDescEstado1='||desc_estado1||' -extRutCotiza1='||rut_cotiza1||' -extNomCotiza1'||nom_cotiza1||' -extDireccion1='||direccion1||' -extGlosaComuna1='||glosa_comuna1||' -extGlosaCiudad1='||glosa_ciudad1||' -extPrevision1='||prevision1||' -extGlosa1='||glosa1||' -extPlan1='||plan1||' -extDsctoPlanilla1='||dscto_planilla1||' -extMontoExec1='||monto_exec1);	

	--xml2:=logapp(xml2,'CGC_BENCERTIF_RECEIVE extApellidoPat='||apell_pat1||' -extApellidoMat='||apell_mat1||' -extNombres='||nombre1||' -extSexo='||sexo1||' -extFechaNacimi='||fecha_nac1||' -extCodEstBen='||cod_est_ben1);
	
	--Beneficiario Certificado
        if (cod_est_ben1='C') or (cod_est_ben1='G') then
                --Resp OK fecha_nac viene asi Nov 12 1
		fecha_nac1:=to_char(to_date(fecha_nac1, 'Mon DD YY'), 'YYYYMMDD');
	...
	xml2:=logapp(xml2,'SALIDA_RUT='||rut_acompa1);
	xml2:=logapp(xml2,'SALIDA_NOMBRE='||nom_acompa1);
