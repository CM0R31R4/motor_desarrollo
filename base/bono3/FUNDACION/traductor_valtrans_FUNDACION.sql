CREATE OR REPLACE FUNCTION traductor_in_valtrans_fundacion(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	extCodFinanciador       := trim(get_campo('EXTCODFINANCIADOR',xml2));
        ...
        xml2:=put_campo(xml2,'SQLINPUT','["ADMIMED.FNDVALTRANS_PKG.FNDVALTRANS", ["$o$STRING", '||extCodFinanciador||', '||exFolioFinanciador||', '||chr(34)||extAccion||chr(34)||', '||chr(34)||extPregunta||chr(34)||', "$o$STRING", "$o$STRING", "$o$STRING"]]'||chr(10)||chr(10));
...
CREATE OR REPLACE FUNCTION traductor_out_valtrans_fundacion(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	if aux1 is null then
		...
		if aux1 is not null then
                        ...
                        xml2:=put_campo(xml2,'ERRORCOD','99');
			xml2:=put_campo(xml2,'ERRORMSG','Fundacion:Error_ValTrans');
                end if;
                xml2    :=put_campo(xml2,'CODIGO_RESP','2');
	...
	xml2:=logapp(xml2,'FUNDACION: RSP_VALTRANS -> extRespuesta1='||respuesta1||' -extCodResp1='||cod_resp1||' -extMensajeResp1='||mensaje_resp1);
