CREATE OR REPLACE FUNCTION traductor_in_bencertif_fundacion(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	--fecha_actual1   := to_char(get_campo('EXTFECHAACTUAL',xml2)::date, 'YYYY/MM/DD');
        --fecha_actual1   := get_campo('EXTFECHAACTUAL',xml2);
	...
	xml2:=put_campo(xml2,'SQLINPUT','["ADMIMED.FNDBENCERTIF_PKG.FNDBENCERTIF", [ "$o$STRING", '||cod_fin1||','||chr(34)||rut_benef1||chr(34)||','||chr(34)||fecha_actual1||chr(34)||', "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$NUMBER", "$o$STRING['||l_array||']", "$o$STRING['||l_array||']" ]]'||chr(10)||chr(10));
	--xml2:=put_campo(xml2,'SQLINPUT','["ADMIMED.FNDBENCERTIF_PKG.FNDBENCERTIF", [ "$o$STRING", '||cod_fin1||','||rut_benef1||','||fecha_actual1||', "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$NUMBER", "$o$STRING['||l_array||']", "$o$STRING['||l_array||']" ]]'||chr(10)||chr(10));
...
CREATE OR REPLACE FUNCTION traductor_out_bencertif_fundacion(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	if aux1 is null then
		...
		if aux1 is not null then
        		...
        		xml2:=put_campo(xml2,'ERRORCOD','99');
			xml2:=put_campo(xml2,'ERRORMSG','Fundacion:Error_Bencertif');
                end if;
                xml2    :=put_campo(xml2,'CODIGO_RESP','2');
	...
	xml2:=logapp(xml2,'FUNDACION: RSP_BENCERTIF -> extApellPat1='||apell_pat1||' -extApellMat1='||apell_mat1||' -extNombre1='||nombre1||' -extSexo1='||sexo1||' -extFechaNac1='||fecha_nac1||' -extCodEstBen1='||cod_est_ben1||' -extDescEstado1='||desc_estado1||' -extRutCotiza1='||rut_cotiza1||' -extNomCotiza1='||nom_cotiza1||' -extDireccion1='||direccion1||' -extGlosaComuna1='||glosa_comuna1||' -extGlosaCiudad1='||glosa_ciudad1||' -extPrevision1='||prevision1||' -extGlosa1='||glosa1||' -extPlan1='||plan1||' -extDsctoPlanilla1='||dscto_planilla1||' -extMontoExec1='||monto_exec1||' -extRutAcompa1='||rut_acompa1||' -extNomAcompa1='||nom_acompa1);

	--fecha_nac1:='21-ENE-33';

        --cod_est_ben1    :=trim(replace(json_field(aux1,'6'),'"',''));
	--Beneficiario Certificado
        if (cod_est_ben1='C') or (cod_est_ben1='G') or (cod_est_ben1='S') then
                --Resp OK fecha_nac viene asi 12-AUG-13
		xml2:=logapp(xml2,'fecha_nac='||fecha_nac1);
		--JCC(20150729): Isapre retorna fecha en Ingles(21-JAN-33) / Espanol(21-ENE-33)
		fecha_nac1:=cambia_mes(fecha_nac1); 
                fecha_nac1:=to_char(to_date(fecha_nac1,'DD Mon YY'),'YYYYMMDD');
		xml2:=logapp(xml2,'fecha_nac0='||fecha_nac1);
	...
	xml2:=logapp(xml2,'fecha_nac1='||fecha_nac1);
	if length(fecha_nac1)>0 then
			...
			xml2:=logapp(xml2,'fecha_nac2='||fecha_nac1);
                end if;
        end if;
