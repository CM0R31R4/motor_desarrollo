CREATE OR REPLACE FUNCTION traductor_in_envbonis_fundacion(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	xml2:=put_campo(xml2,'SQLINPUT','["ADMIMED.FNDENVBONIS_PKG.FNDENVBONIS", ["$o$STRING", '||cod_fin1||', '||chr(34)||extHomNumeroConvenio||chr(34)||', '||chr(34)||extHomLugarConvenio||chr(34)||', '||chr(34)||extSucVenta||chr(34)||', '||chr(34)||rut_conv1||chr(34)||', '||chr(34)||rut_asoc1||chr(34)||', '||chr(34)||extNomPrestador||chr(34)||', '||chr(34)||rut_tratante1||chr(34)||', '||chr(34)||extEspecialidad||chr(34)||', '||chr(34)||rut_benef1||chr(34)||', '||chr(34)||rut_cotiza1||chr(34)||', '||chr(34)||rut_acomp1||chr(34)||', '||chr(34)||rut_emisor1||chr(34)||', '||chr(34)||rut_cajero1||chr(34)||', '||chr(34)||extCodigoDiagnostico||chr(34)||', '||chr(34)||extDescuentoxPlanilla||chr(34)||', '||extMontoExcedente||', '||chr(34)||extFechaEmision||chr(34)||', '||extNivelConvenio||', '||extFolioFinanciador||', '||extMontoValorTotal||', '||extMontoAporteTotal||', '||extMontoCopagoTotal||', '||extNumOperacion||', '||extCorrPrestacion||', '||extTipoSolicitud||', '||chr(34)||extFechaInicio||chr(34)||', '||chr(34)||extUrgencia||chr(34)||', '||chr(34)||extPlan||chr(34)||', '||chr(34)||extLista1||chr(34)||', '||chr(34)||extLista2||chr(34)||', '||chr(34)||extLista3||chr(34)||', "$o$STRING", "$o$STRING"]]'||chr(10)||chr(10));
...
CREATE OR REPLACE FUNCTION traductor_out_envbonis_fundacion(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	if aux1 is null then
		...
		if aux1 is not null then
                        ...
                        xml2:=put_campo(xml2,'ERRORCOD','99');
			xml2:=put_campo(xml2,'ERRORMSG','Fundacion:Error_EnvBonIs');
                end if;
                xml2    :=put_campo(xml2,'CODIGO_RESP','2');
	...
	xml2:=logapp(xml2,'FUNDACION: RSP_ENVBONIS -> extCodResp1='||cod_resp1||' -extMensajeResp1='||mensaje_resp1);
