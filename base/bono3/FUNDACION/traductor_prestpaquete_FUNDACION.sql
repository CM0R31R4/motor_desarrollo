CREATE OR REPLACE FUNCTION traductor_in_prestpaquete_fundacion(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	xml2:=put_campo(xml2,'SQLINPUT','["ADMIMED.FNDPRESTPAQUETE_PKG.FNDPRESTPAQUETE", [ "$o$STRING", '||cod_fin1||','||chr(34)||num_conv1||chr(34)||','||chr(34)||lugar_conv1||chr(34)||','||chr(34)||cod_paquete1||chr(34)||', "$o$STRING","$o$STRING","$o$STRING['||l_array||']","$o$STRING['||l_array||']","$o$NUMBER['||l_array||']" ]]'||chr(10)||chr(10));
...
CREATE OR REPLACE FUNCTION traductor_out_prestpaquete_fundacion(varchar)
returns varchar as
$$
declare
        xml1		alias for $1;
...
begin
	...
	resp1:=replace(get_campo('SQLOUTPUT',xml2),chr(10),'');
        ...
        if aux1 is null then
                ...
                --Otro Error
                        ...
                        xml2:=put_campo(xml2,'ERRORCOD','99');
			xml2:=put_campo(xml2,'ERRORMSG','Fundacion:Error_PrestPaquete');
                end if;
                xml2    :=put_campo(xml2,'CODIGO_RESP','2');
	...
	cod_homol1	:=replace(trim(json_field(aux1,'3')),'.0','');
	item_homol1	:=replace(trim(json_field(aux1,'4')),'.0','');
	--cantidad1	:=replace(trim(json_field(aux1,'5')),'.0','');
	cantidad1	:=trim(json_field(aux1,'5'));
	
	xml2:=logapp(xml2,'FUNDACION: RSP_PRESTPAQUETE -> extCodResp1='||cod_resp1||' -extMensajeResp1='||mensaje_resp1||' -extCodHomol1='||cod_homol1||' -extItemHomol1='||item_homol1||' -extCantidad1='||cantidad1);
