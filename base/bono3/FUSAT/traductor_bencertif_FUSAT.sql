CREATE OR REPLACE FUNCTION traductor_in_bencertif_fusat(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	--fecha_actual1	:=to_char(get_campo('EXTFECHAACTUAL',xml2)::date, 'YYYYMMDD'); 
	...
	--fecha_actual1	:=replace(replace(get_campo('EXTFECHAACTUAL',xml2),'-',''),'/','');
	...
	xml2:=put_campo(xml2,'SQLINPUT','["ADMIMED.FUSBENCERTIF_PKG.FUSBENCERTIF", [ "$o$STRING", '||cod_fin1||','||chr(34)||rut_benef1||chr(34)||','||chr(34)||fecha_actual1||chr(34)||', "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$NUMBER", "$o$STRING['||l_array||']", "$o$STRING['||l_array||']" ]]'||chr(10)||chr(10));
...
CREATE OR REPLACE FUNCTION traductor_out_bencertif_fusat(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	--Si no esta el tag result es porque hay error
        ...
        if aux1 is null then
                ...
                --Otro Error
                        ...
                        xml2:=put_campo(xml2,'ERRORCOD','99');
			xml2:=put_campo(xml2,'ERRORMSG','Fusat:Error_Bencertif');
                end if;
                xml2    :=put_campo(xml2,'CODIGO_RESP','2');
	...
	dscto_planilla1	:=replace(trim(replace(json_field(aux1,'16'),'"','')),'.0','');
	monto_exec1	:=replace(trim(replace(json_field(aux1,'17'),'"','')),'.0','');
	rut_acompa1	:=trim(replace(json_field(aux1,'18'),'", "','","'));
	nom_acompa1	:=trim(replace(json_field(aux1,'19'),'", "','","'));

	xml2:=logapp(xml2,'FUSAT: RSP_BENCERTIF -> extApellPat1='||apell_pat1||' -extApellMat1='||apell_mat1||' -extNombre1='||nombre1||' -extSexo1='||sexo1||' -extFechaNac1='||fecha_nac1||' -extCodEstBen1='||cod_est_ben1||' -extDescEstado1='||desc_estado1||' -extRutCotiza1='||rut_cotiza1||' -extNomCotiza1='||nom_cotiza1||' -extDireccion1='||direccion1||' -extGlosaComuna1='||glosa_comuna1||' -extGlosaCiudad1='||glosa_ciudad1||' -extPrevision1='||prevision1||' -extGlosa1='||glosa1||' -extPlan1='||plan1||' -extDsctoPlanilla1='||dscto_planilla1||' -extMontoExec1='||monto_exec1||' -extRutAcompa1='||rut_acompa1||' -extNomAcompa1='||nom_acompa1);

	--xml2:=logapp(xml2,'FUS_BENCERTIF_RECEIVE extApellidoPat='||apell_pat1||' -extApellidoMat='||apell_mat1||' -extNombres='||nombre1||' -extSexo='||sexo1||' -extFechaNacimi='||fecha_nac1||' -extCodEstBen='||cod_est_ben1);

        --cod_est_ben1    :=trim(replace(json_field(aux1,'6'),'"',''));
	--Beneficiario Certificado
        if (cod_est_ben1='C') or (cod_est_ben1='G') or (cod_est_ben1='S') then
                --Resp OK fecha_nac viene asi 12-AUG-13
                --Resp OK fecha_nac viene asi "19460219"
                fecha_nac1:=to_char(to_date(fecha_nac1,'DD Mon YY'),'YYYYMMDD');
                --Resp OK fecha_nac viene asi "1998/04/02"
		--fecha_nac1:=to_char(to_date(fecha_nac1,'YYYY/MM/DD'),'YYYYMMDD');
                xml2:=put_campo(xml2,'ERRORMSG','');
	elsif (cod_est_ben1='') then
		cod_est_ben1='N';
        else
