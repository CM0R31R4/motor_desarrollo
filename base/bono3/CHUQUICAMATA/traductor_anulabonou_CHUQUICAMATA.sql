CREATE OR REPLACE FUNCTION traductor_in_anulabonou_chuqui(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	xml2    :=put_campo(xml2,'__SECUENCIAOK__','0');
        ...
        xml2:=put_campo(xml2,'SQLINPUT','["ADMIMED.CHUANULABONOU_PKG.CHUANULABONOU", [ "$o$STRING", '||extCodFinanciador||', '||extFolioBono||', '||chr(34)||extIndTratam||chr(34)||', '||chr(34)||extFecTratam||chr(34)||', "$o$STRING", "$o$STRING"]]'||chr(10)||chr(10));
...
CREATE OR REPLACE FUNCTION traductor_out_anulabonou_chuqui(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	--Si no esta el tag result es porque hay error
		...
		if aux1 is not null then
                        ...
                        xml2:=put_campo(xml2,'ERRORCOD','99');
			xml2:=put_campo(xml2,'ERRORMSG','Chuqui:Error_AnulaBonoU');
                end if;
                xml2    :=put_campo(xml2,'CODIGO_RESP','2');
	...
	xml2:=logapp(xml2,'CHUQUICAMATA: RSP_ANULABONOU -> extCodResp1='||cod_resp1||' -extMensajeResp1='||mensaje_resp1);
