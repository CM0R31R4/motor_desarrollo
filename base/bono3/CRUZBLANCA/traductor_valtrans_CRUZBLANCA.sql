CREATE OR REPLACE FUNCTION traductor_in_valtrans_cruzblanca(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	exec_sp:= 'execute prestacion..INGValTrans '||cod_fin1||','||folio_fin1||',"'||accion1||'","'||pregunta1||'", @extRespuesta OUTPUT, @extCodError OUTPUT, @extMensajeError OUTPUT ';
	/*Ejemplo*/ 
	--exec_sp:= exec prestacion..INGValTrans 78, 15721115, 'C', 'V',@extRespuesta output, @extCodError output, @extMensajeError output;
...
CREATE OR REPLACE FUNCTION traductor_out_valtrans_cruzblanca(varchar)
returns varchar as
$$
declare
        xml1		alias for $1;
...
begin
	...
	xml2:=logapp(xml2,'CRUZBLANCA: RSP_VALTRANS -> extRespuesta1='||respuesta1||' -extCodResp1='||cod_resp1||' -extMensajeResp1='||mensaje_resp1);
