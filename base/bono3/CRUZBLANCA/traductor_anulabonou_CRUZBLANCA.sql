CREATE OR REPLACE FUNCTION traductor_in_anulabonou_cruzblanca(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	exec_sp:= 'execute prestacion..INGAnulaBonoU '||cod_fin1||','||folio_bono1||',"'||id_tratam1||'","'||fecha_tratam1||'", @extCodError OUTPUT, @extMensajeError OUTPUT ';	
	/*Ejemplo SP Sybase*/
...
CREATE OR REPLACE FUNCTION traductor_out_anulabonou_cruzblanca(varchar)
returns varchar as
$$
declare
        xml1		alias for $1;
...
begin
	...
	xml2:=logapp(xml2,'CRUZBLANCA: RSP_ANULABONOU -> extCodResp1='||cod_resp1||' -extMensajeResp1='||mensaje_resp1);

	--raise notice 'JCC_ANULABONOU_Receive extCodError=% - extMensajeError=%',cod_resp1,mensaje_resp1;
