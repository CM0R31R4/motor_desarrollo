...
78,"00000042095-000","0070681","P00000000000027"
   exec prestacion..INGPrestPaquete 078, '00000042095-000', '0070681', '819308',
...
l = 'LEPP'
 ...
 insert Log_EntradasPrestPaquete
 (Log_NroTrx, LogFechaTRX,      CodFinanciador, HomNumeroConvenio,
  HomLugarConvenio, CodPaquete)
 values
 (@Folio, getdate(), @extCodFinanciador,  @extHomNumeroConvenio,
  @extHom
LugarConvenio,  @extCodPaquete)
...
char_length(@extHomNumeroConvenio),1))
 ...
 Select @Nomina_Convenio = ConCodNom_ta
 Fr
om   convenio..Convenio
  ...
  begin
   ...
   Select extCodHomologo  = convert(char(12),
        ...
        replicate('0', 3 - char_length(rtrim(ltrim(convert(char(3),CpeCodIte_id))))) +
                            rtrim(ltrim(convert(char(3),CpeCodIte_id)))),
          extItemHomologo = case CpeCodIte_id
                             when   0 then '00'
...
,CpeCanCpe_nn)
  ...
  union
   Select extC
odHomologo  = convert(char(12),
                            rtrim(ltrim(convert(char(10),CpeCodPre_id)))+
                            replicate('0', 3 - char_length(rtrim(ltrim(convert(char(3),CpeCodIte_id))))) +
                            rtrim(ltrim(co
nvert(char(3),CpeCodIte_id)))),
          extItemHomologo = case CpeCodIte_id
                             when   0 then '00'
                             when 101 then '01'
                             when 113 then '02'
 ...
 107 then '09'
          ...
          extCantidad     = convert(int,CpeCanCpe_nn)
  from   prestacion..PaqueteExterno,
          prestacion..PaqueteAsociado,
          prestacion..ComposicionPaque
teExt
   where  PqeCodNom_id = @Nomina_Convenio
     and  PqeCodPaq_id =  @Cod_Paquete
     and  PqeCodNom_id = PqaCodNom_id
...
_id
   ...
   if @@rowcount > 0
    ...
    begin
      Sel
ect extCodHomologo  = convert(char(12),''),
             extItemHomologo = convert(char(2),''),
             extCantidad     = convert(int,0)  --convert(char(2),0)
