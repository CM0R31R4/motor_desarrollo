...
begin
	...
	while true loop
			...
			--Quito la ultima coma (ya que no volvera al ciclo) y se sale
				...
				xml2:=logapp(xml2,'FER_BENCERTIF_RECEIVE i='||i::varchar||' -rut_acompa1='||rut_acompa1||' -nom_acompa1='||nom_acompa1);
			end if;
                        exit;
                end if;
                i:=i+1;
	...
	xml2:=logapp(xml2,'FERROSALUD: RSP_BENCERTIF -> extApellPat1='||apell_pat1||' -extApellMat1='||apell_mat1||' -extNombre1='||nombre1||' -extSexo1='||sexo1||' -extFechaNac1='||fecha_nac1||' -extCodEstBen1='||cod_est_ben1||' -extDescEstado1='||desc_estado1||' -extRutCotiza1='||rut_cotiza1||' -extNomCotiza1='||nom_cotiza1||' -extDireccion1='||direccion1||' -extGlosaComuna1='||glosa_comuna1||' -extGlosaCiudad1='||glosa_ciudad1||' -extPrevision1='||prevision1||' -extGlosa1='||glosa1||' -extPlan1='||plan1||' -extDsctoPlanilla1='||dscto_planilla1||' -extMontoExec1='||monto_exec1);

	--xml2:=logapp(xml2,'FER_BENCERTIF_RECEIVE extApellidoPat='||apell_pat1||' -extApellidoMat='||apell_mat1||' -extNombres='||nombre1||' -extSexo='||sexo1||' -extFechaNacimi='||fecha_nac1||' -extCodEstBen='||cod_est_ben1);
