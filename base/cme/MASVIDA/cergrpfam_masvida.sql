CREATE OR REPLACE FUNCTION traductor_in_cergrpfam_masvida(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	xml2:=proc_parser_input_cergrpfam(xml2);
	...
	--Por ahora en duro
        ...
        exec_sp:= 'EXECUTE dbo.cergrpfam '||get_campo('ANAMTIPODET',xml2)||',
                        ...
                        '||get_campo('ANAMCODPARENT',xml2)||',
                        ['||get_campo('ANAMANTPAR',xml2)||'], @AnamCodError OUTPUT, @AnamMensajeError OUTPUT ';
...
CREATE OR REPLACE FUNCTION traductor_out_cergrpfam_masvida(varchar)
returns varchar as
$$
declare
        xml1            alias for $1;
...
begin
        ...
        raise notice 'JCC_cergrpfam_RECEIVE AnamCodError=% - AnamMensajeError=%',cod_resp1,msj_resp1;
