CREATE OR REPLACE FUNCTION traductor_in_ceremibonopam_masvida(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	xml2:=proc_parser_input_ceremibonopam(xml2);
	...
	exec_sp:= 'EXECUTE dbo.CEREMIBONOPAM '||88||',
			'||get_campo('CTAFOLIOSBONO',xml2)||',
			'||get_campo('CTATIPOPAGO',xml2)||',
			'||get_campo('CTAMTOPAGFIN',xml2)||', @CtaCodError OUTPUT, @CtaMensajeError OUTPUT ';
...
/*
CREATE OR REPLACE FUNCTION traductor_out_ceremibonopam_masvida(varchar)
returns varchar as
$$
declare
        xml1		alias for $1;
...
begin
	...
	raise notice 'JCC_ceremibonopam_RECEIVE CtaCodError=% - CtaMensajeError=%',cod_resp1,msj_resp1;
