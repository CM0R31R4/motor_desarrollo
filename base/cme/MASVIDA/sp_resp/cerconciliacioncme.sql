...
begin
	...
	xml2:=put_campo(xml2,'ERR','N');
        xml2:=put_campo(xml2,'GLOSA','CME Contenida ='||get_campo('TX_WS',xml2)||' - Cod='||get_campo('COD_CME',xml2));
...
begin
	...
	--xml2:=logapp(cerenvcta'_Receive CtaCodError='||cod_error1||' - CtaMensajeError='||msj_error1);
	xml2:=logapp(xml2,get_campo('TX_CME',xml2)||'_Receive CtaCodError='||cod_error1||' - CtaMensajeError='||msj_error1);
