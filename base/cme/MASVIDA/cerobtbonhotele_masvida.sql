CREATE OR REPLACE FUNCTION traductor_in_cerobtbonhotele_masvida(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	xml2:=proc_parser_input_cerobtbonhotele(xml2);
	...
	declare_params:='DECLARE @CtaCodError char(1)
		...
		DECLARE @CtaCoItemPtd		char(3)
		DECLARE @CtaCodPrestHom		char(12)
		DECLARE @CtaCoItemHom		char(3)
	...
	exec_sp:= 'EXECUTE dbo.cerobtbonhotele '||88||',
			['||get_campo('CTARUTCONVENIO',xml2)||'],
			['||get_campo('CTANUMCTA',xml2)||'],
			'||get_campo('CTANUMCOBRO',xml2)||', @CtaCodError OUTPUT, @CtaMensajeError OUTPUT, @CtaCorrRegistro OUTPUT, @CtaCodIntervenPtd OUTPUT, @CtaCodIntervenHom OUTPUT, @CtaNroIntenven OUTPUT, @CtaCodPrestPtd OUTPUT, @CtaCoItemPtd OUTPUT, @CtaCodPrestHom OUTPUT, @CtaCoItemHom OUTPUT, @CtaMtoTotalIsapre OUTPUT, @CtaMtoBonificado OUTPUT, @CtaMtoCopago OUTPUT, @CtaCodPrestRev OUTPUT, @CtaItemPrestRev OUTPUT, @CtaTipoRevalorizacion OUTPUT ';

	out_params:='SELECT @CtaCodError as CtaCodError, @CtaMensajeError as CtaMensajeError, @CtaCorrRegistro as CtaCorrRegistro, @CtaCodIntervenPtd as CtaCodIntervenPtd, @CtaCodIntervenHom as CtaCodIntervenHom, @CtaNroIntenven as CtaNroIntenven, @CtaCodPrestPtd as CtaCodPrestPtd, @CtaCoItemPtd as CtaCoItemPtd, @CtaCodPrestHom as CtaCodPrestHom, @CtaCoItemHom as CtaCoItemHom, @CtaMtoTotalIsapre as CtaMtoTotalIsapre, @CtaMtoBonificado as CtaMtoBonificado, @CtaMtoCopago as CtaMtoCopago, @CtaCodPrestRev as CtaCodPrestRev, @CtaItemPrestRev as CtaItemPrestRev, @CtaTipoRevalorizacion as CtaTipoRevalorizacion ';
...
/*
CREATE OR REPLACE FUNCTION traductor_out_cerobtbonhotele_masvida(varchar)
returns varchar as
$$
declare
        xml1		alias for $1;
	...
	co_item_ptd1	varchar ='';
	cod_prest_hom1	varchar ='';
	co_item_hom1	varchar ='';
	...
	corr_reg_aux1       varchar;
        ...
        co_item_ptd_aux1    varchar;
        cod_prest_hom_aux1  varchar;
        co_item_hom_aux1    varchar;
...
begin
	...
	while true loop
		...
		co_item_ptd_aux1	:=trim(get_campo('CTACOITEMPTD_'||i::varchar,xml2));
		cod_prest_hom_aux1	:=trim(get_campo('CTACODPRESTHOM_'||i::varchar,xml2));
		co_item_hom_aux1	:=trim(get_campo('CTACOITEMHOM_'||i::varchar,xml2));
		...
		tipo_rev_aux1		:=trim(get_campo('CTATIPOREVALORIZACION_'||i::varchar,xml2));
		   ...
		   length(nro_inten_aux1)>0 or length(cod_prest_ptd_aux1)>0 or length(co_item_ptd_aux1)>0 or
                   length(cod_prest_hom_aux1)>0 or length(co_item_hom_aux1)>0 or length(mto_tot_isap_aux1)>0 or 
		   length(mto_bonif_aux1)>0 or length(mto_copag_aux1)>0 or length(cod_prest_rev_aux1)>0 or 
		   length(item_prest_rev_aux1)>0 or length(tipo_rev_aux1)>0 then
                        ...
                        co_item_ptd1  	:=co_item_ptd1||co_item_ptd_aux1||',';
                        cod_prest_hom1 	:=cod_prest_hom1||cod_prest_hom_aux1||',';
                        co_item_hom1    :=co_item_hom1||co_item_hom_aux1||',';
			...
			if length(corr_reg1)>0 or length(cod_inter_ptd1)>0 or length(cod_inter_hom1)>0 or
                   	   length(nro_inten1)>0 or length(cod_prest_ptd1)>0 or length(co_item_ptd1)>0 or
                   	   length(cod_prest_hom1)>0 or length(co_item_hom1)>0 or length(mto_tot_isap1)>0 or
                   	   length(mto_bonif1)>0 or length(mto_copag1)>0 or length(cod_prest_rev1)>0 or
                   	   length(item_prest_rev1)>0 or length(tipo_rev1)>0 then
                                ...
                                co_item_ptd1	:=substring(co_item_ptd1,1,length(co_item_ptd1)-1);
                                cod_prest_hom1 	:=substring(cod_prest_hom1,1,length(cod_prest_hom1)-1);
                                co_item_hom1 	:=substring(co_item_hom1,1,length(co_item_hom1)-1);
	...
	raise notice 'JCC_cerobtbonhotele_Receive CtaCodError=% - CtaMensajeError=%',cod_resp1,msj_resp1;
	raise notice 'JCC_cerobtbonhotele_Receive corr_reg1=% - cod_inter_ptd1=%',corr_reg1,cod_inter_ptd1;
...
--	xml2:=put_campo(xml2,'CTACORRREGISTRO',		corr_reg1);
	...
	xml2:=put_campo(xml2,'CTACOITEMPTD',		co_item_ptd1);
	xml2:=put_campo(xml2,'CTACODPRESTHOM',		cod_prest_hom1);
	xml2:=put_campo(xml2,'CTACOITEMHOM',		co_item_hom1);
