...
CREATE OR REPLACE FUNCTION proc_parser_input_cerenvcta(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	xml2:=  put_campo(xml2,'CTAFECEMISION',		substring(replace(get_campo('CTAFECEMISION',xml2),'-',''),1,17));
	xml2:=  put_campo(xml2,'CTAFECINGRESO',		substring(replace(get_campo('CTAFECINGRESO',xml2),'-',''),1,17));
	xml2:=  put_campo(xml2,'CTAHORAINGRESO',	coalesce(nullif(get_campo('CTAHORAINGRESO',xml2),''),'0'));
	xml2:=  put_campo(xml2,'CTAFECCORTE',		substring(replace(get_campo('CTAFECCORTE',xml2),'-',''),1,17));
	...
	xml2:=  put_campo(xml2,'CTAFECFUR',		substring(replace(get_campo('CTAFECFUR',xml2),'-',''),1,17));
	...
	xml2:=  put_campo(xml2,'CTACODDIAGPRIN',	coalesce(nullif(get_campo('CTACODDIAGPRIN',xml2),''),'0'));
