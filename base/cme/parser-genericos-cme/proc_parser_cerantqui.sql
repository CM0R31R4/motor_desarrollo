CREATE OR REPLACE FUNCTION proc_parser_input_cerantqui(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	xml2:=  put_campo(xml2,'ANAMCODANTQUIR',	coalesce(nullif(get_campo('ANAMCODANTQUIR',xml2),''),'0'));
	xml2:=  put_campo(xml2,'ANAMANTQUIR',		coalesce(nullif(get_campo('ANAMANTQUIR',xml2),''),'0'));
	xml2:=  put_campo(xml2,'ANAMFECINTER',		coalesce(nullif(get_campo('ANAMFECINTER',xml2),''),'0'));
