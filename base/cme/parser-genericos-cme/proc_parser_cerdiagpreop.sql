CREATE OR REPLACE FUNCTION proc_parser_input_cerdiagpreop(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	xml2:=  put_campo(xml2,'PROCODDIAGPRE',    coalesce(nullif(get_campo('PROCODDIAGPRE',xml2),''),'0'));
	xml2:=  put_campo(xml2,'PROGLODIAGPRE',    coalesce(nullif(get_campo('PROGLODIAGPRE',xml2),''),'0'));
