CREATE OR REPLACE FUNCTION traductor_in_cerdetprot_masvida(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	xml2:=proc_parser_input_cerdetprot(xml2);
	...
	exec_sp:= 'EXECUTE dbo.cerdetprot '||get_campo('PROTIPODET',xml2)||',
			...
			'||get_campo('PRONUMCOR',xml2)||',
			['||get_campo('PRODESCRIPCION',xml2)||'], @ProCodError OUTPUT, @ProMensajeError OUTPUT ';
...
/*CREATE OR REPLACE FUNCTION traductor_out_cerdetprot_masvida(varchar)
returns varchar as
$$
declare
        xml1		alias for $1;
...
begin
	...
	raise notice 'JCC_CERDETPROT_RECEIVE ProCodError=% - ProMensajeError=%',cod_resp1,msj_resp1;
