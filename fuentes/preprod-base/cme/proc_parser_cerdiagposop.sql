CREATE OR REPLACE FUNCTION proc_parser_input_cerdiagposop(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	xml2:=  put_campo(xml2,'PROCODDIAGPOST',    coalesce(nullif(get_campo('PROCODDIAGPOST',xml2),''),'0'));
	xml2:=  put_campo(xml2,'PROGLODIAGPOST',    coalesce(nullif(get_campo('PROGLODIAGPOST',xml2),''),'0'));
