...
--Actualiza tabla de control del motor
...
declare
	...
	cont1           integer='1';
        ...
        folios1         varchar='';
	tabla1		varchar;
	status_bono1	varchar;
	i		integer;
begin
	xml2:=xml1;

	--Actualiza tabla de control del motor
	i:=actualiza_control_motor(get_campo('__PROCESOXML__',xml2),get_campo('__PROC_ACTIVOS__',xml2));
	--xml2:=logapp(xml2,'Tipo_TX='||get_campo('TX_WS',xml2));
	...
	if (get_campo('REQUEST_METHOD',xml2)='GET' and 
		...
		xml2:=proc_wsdl_3002(xml2);
		xml2:=put_campo(xml2,'__SECUENCIAOK__','0');
		return xml2;
	end if;

	--Para las confirmaciones, las sacamos de la cola despues de 1 dia
	if (get_campo('TX_WS',xml2) in ('envbonis_real','envbono_real')) then
		xml2:=logapp(xml2,'*__COLA_MOTOR__='||get_campo('__COLA_MOTOR__',xml2));
		xml2:=logapp(xml2,'*__ID_DTE__='||get_campo('__ID_DTE__',xml2));
		--Verifico cuanto tiempo lleva en la cola este requerimiento
		xml2:=logapp(xml2,'FECHA_INGRESO_COLA='||get_campo('FECHA_INGRESO_COLA',xml2));	
		--Si han pasado mas de 8 horas lo saco de la cola
		if (now()-get_campo('FECHA_INGRESO_COLA',xml2)::timestamp>interval '8 hour') then
			xml2:=logapp(xml2,'Se borra de la cola por Timeout');
			id1:=get_campo('__ID_DTE__',xml2);
			xml2	:=put_campo(xml2,'__SECUENCIAOK__','0');
			--Inserto en cola de errores
			execute 'insert into confirma_bono_errores (fecha,estado,reintentos,prioridad,xml_in,financiador,tx,id) select fecha,estado,reintentos,prioridad,xml_in,financiador,tx,id from confirma_bono_'||get_campo('__COLA_MOTOR__',xml2)||' where id='||id1;
	...
	/*--Si solo fallo la confirmacion del bono
        if (get_campo('STATUS_BONO',xml2)='NK') then
                xml2:=logapp(xml2,'Reprocesa la confirmacion del Bono');
		--Toma el Input_Ori y lo hace Input
		xml2:=put_campo(xml2,'INPUT',get_campo('INPUT_ORI',xml2));
		--Continua como si fuera una trx de confirmacion normal.
	end if;*/
	...
	if (get_campo('TX_WS',xml2) in ('anulabonou')) then
		...
		--TODO: Caso 12408810. Folio tiene dos registros 
		--SELECT * INTO stBono FROM tx_bono3 WHERE folio=folio_anular1 and cod_fin=id_fin1 and tipo_tx in ('envbonis_real','envbono_real');
		SELECT * INTO stBono FROM tx_bono3 WHERE folio=folio_anular1 and cod_fin=id_fin1 and tipo_tx in ('envbonis_real','envbono_real') order by fecha_in_tx desc;
	...
	*/
	...
	--Si es una confirmacion real procesamos el SQLINPUT cambiando los caracteres de control
	/*if (get_campo('TX_WS',xml2) in ('envbonis_real','envbono_real')) then
		xml2:=logapp(xml2,'Convierte caracteres escape');
		xml2:=logapp(xml2,get_campo('SQLINPUT',xml2));
		xml2:=put_campo(xml2,'SQLINPUT',elimina_tildes(convert_escape(get_campo('SQLINPUT',xml2))));
...
*/
	...
	--TODO: Servicio Dummies. Nunca va al Financiador para este servicio en PROD.
        if (tipo_tx1='envbonis' or tipo_tx1='envbono')  then
                xml2:=logapp(xml2,'Envbonis: Confirmacion Folio Bono='||get_campo('EXTFOLIOFINANCIADOR',xml2)||' OK');
...
begin
	...
	--Log diferenciado
	if (get_campo('TAG_RESPUESTA_HEX',xml2)='SI') then
		xml2:=logapp(xml2,'Respuesta Financiador='||decode(get_campo('SQLOUTPUT',xml2),'hex')::varchar);
	else
		xml2:=logapp(xml2,'Respuesta Financiador='||get_campo('SQLOUTPUT',xml2));
        end if;


	--Si la respuesta viene en HEX
	xml2:=logapp(xml2,'Respuesta="'||get_campo('MENSAJE_RESP',xml2)||'" Codigo='||get_campo('CODIGO_RESP',xml2)||' extCodError='||get_campo('extCodError',xml2)||' extMensajeError='||get_campo('extMensajeError',xml2)||' Timeout='||timeout1::varchar);
	...
	xml2:=logapp(xml2,'__STS_ERROR_SOCKET__='||get_campo('__STS_ERROR_SOCKET__',xml2)||' API_CODRESPUESTA='|| get_campo('API_CODRESPUESTA',xml2)||' ESTADO_WS='||get_campo('ESTADO_WS',xml2)||' API_DESCRIPCION_MSG='||get_campo('API_DESCRIPCION_MSG',xml2));
	...
	--Si es una confirmacion de Bono envbonis_real o enbono_real vamos a confirmar al percona 
