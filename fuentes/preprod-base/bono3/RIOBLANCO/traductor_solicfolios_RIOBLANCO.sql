CREATE OR REPLACE FUNCTION traductor_in_solicfolios_rioblanco(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	xml2    :=put_campo(xml2,'__SECUENCIAOK__','0');
        ...
        xml2:=put_campo(xml2,'SQLINPUT','["ADMIMED.RBLSOLICFOLIOS_PKG.RBLSOLICFOLIOS", ["$o$STRING", '||extCodFinanciador||', '||extNumFolios||', "$o$STRING", "$o$STRING", "$o$NUMBER['||l_array||']"]]'||chr(10)||chr(10));
...
CREATE OR REPLACE FUNCTION traductor_out_solicfolios_rioblanco(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
	...
	xml2    :=put_campo(xml2,'ESTADO_TX','TERMINADO_OK');
        ...
        if aux1 is null then
                ...
                if aux1 is not null then
                        ...
                        xml2:=put_campo(xml2,'ERRORCOD','99');
			xml2:=put_campo(xml2,'ERRORMSG','RioBlanco:Error_SolicFolios');
                end if;
                xml2    :=put_campo(xml2,'CODIGO_RESP','2');
	 ...
	 --Parseo la Respuesta
        ...
        xml2:=logapp(xml2,'RIOBLANCO: RSP_SOLICFOLIOS -> extCodError='||cod_resp1||' -extMensajeError='||mensaje_resp1||' -extFoliosDevueltos='||folios_resp1);
