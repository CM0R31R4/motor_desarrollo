CREATE OR REPLACE FUNCTION traductor_in_validacat_vidatres(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
CREATE OR REPLACE FUNCTION traductor_out_validacat_vidatres(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	resp1:=replace(get_campo('SQLOUTPUT',xml2),chr(10),'');
        ...
        if aux1 is null then
                ...
                --Otro Error
                        ...
                        xml2:=put_campo(xml2,'ERRORCOD','99');
			xml2:=put_campo(xml2,'ERRORMSG','VidaTres:Error_ValidaCat');
                end if;
                xml2    :=put_campo(xml2,'CODIGO_RESP','2');
	...
	xml2:=logapp(xml2,'VIDATRES: RSP_VALIDACAT -> extRespuestaCat='||cod_resp1||' -extMensajeCat='||mensaje_resp1);
