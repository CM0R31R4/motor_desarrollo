	...
	xml2    :=put_campo(xml2,'ESTADO_TX','TERMINADO_OK');
        ...
        while true loop
                ...
                if length(valor_aux1)>0 or length(aporte_aux1)>0 or length(copago_aux1)>0 or length(interno_aux1)>0  then
					...
					xml2:=logapp(xml2,'CCHC: RSP_VALORVARI -> Falla Obtencion 2doGrupo del Recordset - CodMotor= '||get_campo('CODIGO_MOTOR',xml2));
                                end if;
