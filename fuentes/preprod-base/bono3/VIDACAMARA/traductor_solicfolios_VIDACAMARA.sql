CREATE OR REPLACE FUNCTION traductor_in_solicfolios_vidacamara(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	exec_cmd:= 'EXEC dbo.NMDSolicFolios '||ext_cod_financiador||','||ext_cantidad_folios||', @extCodError OUTPUT,@extMensajeError OUTPUT; ';
...
CREATE OR REPLACE FUNCTION traductor_out_solicfolios_vidacamara(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	--Parseo la Respuesta
        cod_resp1       :=trim(get_campo('EXTCODERROR',xml2));
        mensaje_resp1   :=trim(get_campo('EXTMENSAJEERROR',xml2));
        folios_resp1    :=trim(get_campo('EXFOLIOSDEVUELTOS',xml2));
	...
	while true loop
        ...
        xml2:=logapp(xml2,'VIDACAMARA: RSP_SOLICFOLIOS -> extCodError='||cod_resp1||' -extMensajeError='||mensaje_resp1||' -extFoliosDevueltos='||folios_resp1);
