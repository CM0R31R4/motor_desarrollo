CREATE OR REPLACE FUNCTION traductor_in_enrola_vidacamara(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	xml2:=put_campo(xml2,'SQLINPUT',declare_params||' EXEC dbo.NMDEnrola '||cod_fin1||',"'||rut_enrola1||'","'||rut_benef1||'",'||out_params);
...
CREATE OR REPLACE FUNCTION traductor_out_enrola_vidacamara(varchar)
returns varchar as
$$
declare
        xml1            alias for $1;
...
begin
	...
	xml2:=logapp(xml2,'VIDACAMARA: RSP_ENROLA -> extValido='||cod_resp1||' -extNombreComp='||nombre1);	
