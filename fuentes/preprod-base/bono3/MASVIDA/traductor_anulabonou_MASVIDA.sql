CREATE OR REPLACE FUNCTION traductor_in_anulabonou_masvida(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	exec_sp:= 'execute dbo.MASAnulaBonoU '||cod_fin1||','||folio1||',"'||id_tratam1||'","'||fecha_tratam1||'", @extCodError OUTPUT, @extMensajeError OUTPUT; ';	
...
CREATE OR REPLACE FUNCTION traductor_out_anulabonou_masvida(varchar)
returns varchar as
$$
declare
        xml1		alias for $1;
...
begin
	...
	xml2:=logapp(xml2,'MAS_VIDA: RSP_ANULANONO -> extCodError='||cod_resp1||' -extMensajeError='||mensaje_resp1);
