CREATE OR REPLACE FUNCTION bono3.traductor_in_envbonis_masvida(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	rut_asoc1	:=split_part(ltrim(get_campo('EXTRUTASOCIADO',xml2),'0'),'-',1);      	-- char(12),
	...
	especialidad1   :=coalesce(nullif(get_campo('EXTESPECIALIDAD',xml2),''),'0');	 -- char(10), 
	rut_benef1	:=split_part(ltrim(get_campo('EXTRUTBENEFICIARIO',xml2),'0'),'-',1);	-- char(12),             
	rut_cotiza1	:=split_part(ltrim(get_campo('EXTRUTCOTIZANTE',xml2),'0'),'-',1);      	-- char(12),
	rut_acomp1 	:=split_part(ltrim(get_campo('EXTRUTACOMPANANTE',xml2),'0'),'-',1);    	-- char(12),
	rut_emisor1	:=split_part(ltrim(get_campo('EXTRUTEMISOR',xml2),'0'),'-',1);         	-- char(12),
	rut_cajero1	:=split_part(ltrim(get_campo('EXTRUTCAJERO',xml2),'0'),'-',1);         	-- char(12),
	...
	fecha_emision1	:=replace(coalesce(nullif(get_campo('EXTFECHAEMISION',xml2),''),'0'),'-','');--YYYYMMDD
	...
	--Rut con formato
        ...
        rut_asoc1	:=lpad(rut_asoc1,10,'0')||'-'||motor_modulo11(rut_asoc1);
        ...
        rut_cotiza1	:=lpad(rut_cotiza1,10,'0')||'-'||motor_modulo11(rut_cotiza1);
        rut_acomp1	:=lpad(rut_acomp1,10,'0')||'-'||motor_modulo11(rut_acomp1);
        rut_emisor1	:=lpad(rut_emisor1,10,'0')||'-'||motor_modulo11(rut_emisor1);
        rut_cajero1	:=lpad(rut_cajero1,10,'0')||'-'||motor_modulo11(rut_cajero1);
...
CREATE OR REPLACE FUNCTION bono3.traductor_out_envbonis_masvida(varchar)
returns varchar as
$$
declare
        xml1    	alias for $1;
