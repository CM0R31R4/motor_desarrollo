CREATE OR REPLACE FUNCTION traductor_in_enrola_fundacion(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	--Valida formato del Rut
        ...
        xml2:=put_campo(xml2,'SQLINPUT','["ADMIMED.FNDENROLA_PKG.FNDENROLA", [ "$o$STRING", '||cod_fin1||', '||chr(34)||rut_enrola1||chr(34)||', '||chr(34)||rut_benef1||chr(34)||', "$o$STRING", "$o$STRING" ]]'||chr(10)||chr(10));
...
CREATE OR REPLACE FUNCTION traductor_out_enrola_fundacion(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	if aux1 is null then
		...
		if aux1 is not null then
                        ...
                        xml2:=put_campo(xml2,'ERRORCOD','99');
			xml2:=put_campo(xml2,'ERRORMSG','Fundacion:Error_Enrola');
                end if;
                xml2    :=put_campo(xml2,'CODIGO_RESP','2');
	...
	--Parseo la Respuesta
        ...
        xml2:=logapp(xml2,'FUNDACION: RSP_ENROLA -> extValido='||cod_resp1||' -extNombreComp='||nombre1);
