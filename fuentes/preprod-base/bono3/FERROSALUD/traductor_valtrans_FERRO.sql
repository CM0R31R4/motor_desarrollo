CREATE OR REPLACE FUNCTION traductor_in_valtrans_ferrosalud(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	xml2:=put_campo(xml2,'SQLINPUT',declare_params||' execute dbo.FERValtrans '||ext_cod_financiador||','||ext_folio_financiador||',"'||ext_accion||'","'||ext_pregunta||'", '||output_params);
        return xml2;
...
CREATE OR REPLACE FUNCTION traductor_out_valtrans_ferrosalud(varchar)
returns varchar as
$$
declare
        xml1            alias for $1;
...
begin
	...
	xml2:=logapp(xml2,'FERROSALUD: RSP_VALTRANS -> extRespuesta='||respuesta1||' -extCodError='||cod_resp1||' -extMensajeError='||mensaje_resp1);
