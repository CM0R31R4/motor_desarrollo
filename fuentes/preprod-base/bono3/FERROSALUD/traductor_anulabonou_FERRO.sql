CREATE OR REPLACE FUNCTION traductor_in_anulabonou_ferrosalud(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	exec_sp:= 'execute dbo.FERAnulaBonoU '||cod_fin1||','||folio1||',"'||id_tratam1||'","'||fecha_tratam1||'", @extCodError OUTPUT, @extMensajeError OUTPUT; ';	
	/*Ejemplo SP MSQL SERVER*/
	--exec dbo.FERAnulaBonoU 88, 34, 'N', '04/20/2001', @extCodError output, @extMensajeError output;
...
CREATE OR REPLACE FUNCTION traductor_out_anulabonou_ferrosalud(varchar)
returns varchar as
$$
declare
        xml1		alias for $1;
...
begin
	...
	xml2:=logapp(xml2,'FERROSALUD: RSP_ANULANONO -> extCodError='||cod_resp1||' -extMensajeError='||mensaje_resp1);
