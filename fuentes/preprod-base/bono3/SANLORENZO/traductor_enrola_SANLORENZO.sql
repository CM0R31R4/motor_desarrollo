CREATE OR REPLACE FUNCTION traductor_in_enrola_sanlorenzo(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	xml2:=put_campo(xml2,'SQLINPUT','["ADMIMED.SNLENROLA_PKG.SNLENROLA", [ "$o$STRING", '||cod_fin1||','||chr(34)||rut_enrola1||chr(34)||','||chr(34)||rut_benef1||chr(34)||', "$o$STRING", "$o$STRING" ]]'||chr(10)||chr(10));
...
CREATE OR REPLACE FUNCTION traductor_out_enrola_sanlorenzo(varchar)
returns varchar as
$$
declare
        xml1		alias for $1;
...
begin
	 ...
	 --Si no esta el tag result es porque hay error
        ...
        if aux1 is null then
                ...
                --Otro Error
                        ...
                        xml2:=put_campo(xml2,'ERRORCOD','99');
			xml2:=put_campo(xml2,'ERRORMSG','SanLorenzo:Error_Enrola');
                end if;
                xml2    :=put_campo(xml2,'CODIGO_RESP','2');
	...
	xml2:=logapp(xml2,'SANLORENZO: RSP_ENROLA -> extValido='||cod_resp1||' -extNombreComp='||nombre1);
