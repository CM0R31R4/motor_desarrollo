CREATE OR REPLACE FUNCTION traductor_in_datosprest_cruzdelnorte(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	xml2:=put_campo(xml2,'SQLINPUT','["ISAPRE_ADM.CDNDATOSPREST_PKG.BANDATOSPREST", [ "$o$STRING", '||cod_fin1||','||chr(34)||rut_conv1||chr(34)||','||chr(34)||cod_sucur1||chr(34)||', "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING" ]]'||chr(10)||chr(10));	
...
CREATE OR REPLACE FUNCTION traductor_out_datosprest_cruzdelnorte(varchar)
returns varchar as
$$
declare
        xml1		alias for $1;
...
begin
	...
	--Si no esta el tag result es porque hay error
        ...
        if aux1 is null then
                ...
                --Otro Error
                        ...
                        xml2:=put_campo(xml2,'ERRORCOD','99');
			xml2:=put_campo(xml2,'ERRORMSG','CruzDelNorte:Error_DatosPrest');
                end if;
                xml2    :=put_campo(xml2,'CODIGO_RESP','2');
	...
	xml2:=logapp(xml2,'CRUZDELNORTE: RSP_DATOSPREST -> extEstConvenio='||est_conv1||' -extNivel='||nivel1||' -extTipoPrestador='||tipo_prest1||' -extCodEspecialidades='||cod_espec1||' -extCodProfesiones='||cod_prof1||' -extAnosAntiguedad='||antiguedad1);
