...
---------------------- 
   ...
   (CB,AS), y de retornar información de este; entre otros la aceptación o bloqueo, vigencias, plan, 
   etc.
 En caso de menores de edad se debe devolver en caso de ser requeridos la lista de acompañantes 
...
aComuna        char(30), 
   ...
   @extSexo output, 
                                ...
                                @extNomCotizante output, @extDirPacient
e output, 
...
, 
 ...
 Select @FechaEntrada = convert(smalldatetime,convert(char(10)
,@extFechaActual,101)) 
...
Select @MarcaFinanciador = MarEmpIsa_cr
 ...
 if @FechaEntrada = @Hoy  --/
/Se ha Solicitado verificacion para la venta de un bono al dÝa 
  begin 
...
g_Certificaciones 
   ...
   Select  @@spid, 
           BenRutB
en_nn, BenMarCon_id, BenIniVig_fc, BenTerVig_fc, 
   ...
   begin 
      if @NroFilas = 0 
         ...
         Select 
              ...
              @extGlosa      = 'Benef. vig. en más de un cto.', --'TRANSACCION INCO
MPLETA/ASISTIR A ISAPRE' 
              @ResCert       = 'MC' 
...
oNumFol_nn 
      ...
      insert Log_Certificaciones 
           (LceCorCer_id, LceMarBen_ta, LceRutBen_ta, LceFecCer_fc, 
LceDveBen_cr, 
            LceRutCon_ta, LceDveCon_cr, LceNumCon_ta, LceCodPla_ta, 
            LceResCer_re) 
      values (@Folio, @Marca,        @RutBen,      getdate(),    '0', 
...
enMarCon_id 
     ...
     -- Control del financiador v/s asegurador de beneficiar
io
...
0, 'A1', @ResCert) 
     ...
     Select @extRutCotizante = replicate('0',12 - char_length(lt
rim(rtrim(convert(char(10),BenRutCot_id)))+'-'+ltrim(rtrim(BenDvCot_cr))))+ 
...
0' 
     ...
     Select @extPrevision = case ConTipTra_fl 
      ...
      when '4' then 'V' 
            ...
            @extPlan               = case 
                                      when ((ConIniPAn_pe <= @Hoy)and(@Hoy < ConIniPla_pe)) then ConPlaAnt_ta 
                                      when
 ConIniPla_pe <= @Hoy then ConPlaVig_ta 
                                      else 'PLAN NOFOUND' 
                                     end, 
...
om   contrato..Contrato 
    ...
    if rtrim(@ValAtr) = 'SI'   -- Habilitado para operar con Excedentes 
     begin 
          exec @Result_Exe = ingreso
..SelSaldoDispContExcedente 
                                 @RutCotizante 
                                ,'IN' 
                                ,@extMontoExcedente output 
                                ,@Exc_Contable      output 
...
--  fin FR 19057 
     ...
     select  top 1
             @extDirPaciente = ltrim(rtrim(DIR_CALLE_DS)) + case when DIR_NUMERO_DS = '99999     ' then '' else +' #'+ ltrim(rtrim(DIR_NUMERO_DS)) end +' '+    
                               isNull(ltrim(rtrim(DIR_R
ESTO_DS)), '')
     ...
     if @LocaDir is Null
        Select 
@extDirPaciente = ltrim(rtrim(DirDir_ds))+' '+ltrim(rtrim(DirSecDir_ds)), 
               @LocaDir        = DirLoc_ta 
...
Ciudad = LocDesLoc_ds 
     ...
     --// Se entiende por autorizados a cargas de un contrato mßs el cotizante y 
     --// en el caso de los contratos matrimoniales, tam
bien al conyuge y sus cargas. 
...
arca 
     ...
     if ((@TipPla = 'MA')and(@RutCny > 0)and(@RutCny is not null)) 
      begin 
       ...
       from   tempdb02..IBC_DatosConyuge 
       where  spid         = @@spid 
 ...
 BenMarCon_id = @Marca 
       ...
       if @@rowcount > 0 
        begin 
         ...
         insert tempdb02..IBC_Autorizados 
         Select @@spid, 
                ltrim(rtrim(convert(char(12),B
enRutBen_nn)))+'-'+BenDvBen_cr, 
                ltrim(rtrim(BenApePat_ds))+' '+ 
                ltrim(rtrim(BenApeMat_ds))+' '+ 
                ltrim(rtrim(BenNom_ds)) 
         From   contrato..Beneficiario 
...
stado         = 'NO VIGENTE', 
      ...
      begin 
       if((dateadd(mm,2,@Sucripcion) <= @Hoy)and(@Hoy < @IanBen_
pe)) 
        begin 
         if ((@BenIniVig_fc <= @Hoy)and(@Hoy  <= @BenTerVig_fc)) 
          begin 
...
, 
     ...
     /* 
     decl
are @EstaEnMora  char(1), @ImpCertificado char(1), @Gestionable char(1), 
             @BloqueoxCob char(1), @ContadorEnt    int 
 
     --//exec @ErrorCode = ingreso..Verifica_MoraCot @RutCotizante, @BenDvCot_cr, 
     exec @ErrorCode = ingreso..Verifica
_MoraIMED @RutCotizante, @BenDvCot_cr, 
                                                  @Marca, @NroContrato, 
                                                  'CSALUD00', '130001', 
                                                  @EstaEnMora     out
put, 
...
', 
  ...
  begin --// (DÝa distinto a Hoy, Se ha Solicitado verificacion para la venta de un bono retroactivo 
...
'', 
          ...
          @extGlosa              = 'Benef. no vigente', --'Fecha vta. no corresponde al dia de hoy.' 
          @extPlan               = '', 
...
-- Este control se manejará
 en el servicio : prestacion..ingcoptran 
...
xtSexo               = '', 
   ...
   @extDirPaciente        = '', 
          ...
          @extDescEstado         = 'BLOQUEADO', 
          @e
xtGlosa              = 'Error en grabacion de Log', 
          @extPlan               = '', 
          @extDescuentoxPlanilla = '', 
          @extMontoExcedente     = 0 
