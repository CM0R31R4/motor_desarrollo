CREATE OR REPLACE FUNCTION traductor_in_valorizi_banmedica(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	*/
	...
	xml2:=put_campo(xml2,'SQLINPUT','["INFOMEDICA.BANVALORIZI_PKG.BANVALORIZI", [ "$o$STRING", '||cod_fin1||','||chr(34)||num_conv1||chr(34)||','||chr(34)||lugar_conv1||chr(34)||','||chr(34)||suc_venta1||chr(34)||','||chr(34)||rut_conv1||chr(34)||','||chr(34)||rut_tratante1||chr(34)||','||chr(34)||especialidad1||chr(34)||','||chr(34)||rut_solic1||chr(34)||','||chr(34)||rut_benef1||chr(34)||','||chr(34)||tratamiento1||chr(34)||','||chr(34)||cod_diag1||chr(34)||','||nivel_conv1||','||chr(34)||urgencia1||chr(34)||','||chr(34)||lista1||chr(34)||','||chr(34)||lista2||chr(34)||','||chr(34)||lista3||chr(34)||','||chr(34)||lista4||chr(34)||','||chr(34)||lista5||chr(34)||','||chr(34)||lista6||chr(34)||','||num_prestad1||',"$o$STRING","$o$STRING","$o$STRING","$o$STRING","$o$STRING","$o$STRING","$o$STRING","$o$STRING","$o$NUMBER['||l_array||']","$o$NUMBER['||l_array||']","$o$NUMBER['||l_array||']","$o$STRING['||l_array||']","$o$NUMBER['||l_array||']","$o$NUMBER['||l_array||']","$o$NUMBER['||l_array||']","$o$NUMBER['||l_array||']","$o$NUMBER['||l_array||']","$o$NUMBER['||l_array||']","$o$NUMBER['||l_array||']","$o$NUMBER['||l_array||']","$o$NUMBER['||l_array||']","$o$NUMBER['||l_array||']" ]]'||chr(10)||chr(10));
...
CREATE OR REPLACE FUNCTION traductor_out_valorizi_banmedica(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
	...
	resp1:=replace(get_campo('SQLOUTPUT',xml2),chr(10),'');
        ...
        if aux1 is null then
                ...
                --Otro Error
                        ...
                        xml2:=put_campo(xml2,'ERRORCOD','99');
			xml2:=put_campo(xml2,'ERRORMSG','Banmedica:Error_ValorIzi');
                end if;
                xml2    :=put_campo(xml2,'CODIGO_RESP','2');
