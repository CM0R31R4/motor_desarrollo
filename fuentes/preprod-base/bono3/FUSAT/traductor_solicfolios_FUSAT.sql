CREATE OR REPLACE FUNCTION traductor_in_solicfolios_fusat(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	xml2:=put_campo(xml2,'SQLINPUT','["ADMIMED.FUSSOLICFOLIOS_PKG.FUSSOLICFOLIOS", [ "$o$STRING", '||cod_fin1||','||num_folios1||', "$o$STRING","$o$STRING","$o$NUMBER['||l_array||']" ]]'||chr(10)||chr(10));       
	--xml2:=put_campo(xml2,'SQLINPUT','["ADMIMED.FUSSOLICFOLIOS_PKG.FUSSOLICFOLIOS", [ "$o$STRING", '||cod_fin1||','||chr(34)||num_folios1||chr(34)||', "$o$STRING","$o$STRING","$o$STRING['||l_array||']" ]]'||chr(10)||chr(10));       
...
CREATE OR REPLACE FUNCTION traductor_out_solicfolios_fusat(varchar)
returns varchar as
$$
declare
        xml1		alias for $1;
...
begin
	...
	resp1:=replace(get_campo('SQLOUTPUT',xml2),chr(10),'');
        ...
        if aux1 is null then
                ...
                --Otro Error
                        ...
                        xml2:=put_campo(xml2,'ERRORCOD','99');
			xml2:=put_campo(xml2,'ERRORMSG','Fusat:Error_Solicfolios');
                end if;
                xml2    :=put_campo(xml2,'CODIGO_RESP','2');
	...
	xml2:=logapp(xml2,'FUSAT: RSP_SOLICFOLIOS -> extCodError='||cod_resp1||' -extMensajeError='||mensaje_resp1||' -extFoliosDevueltos='||folios_resp1);
