CREATE OR REPLACE FUNCTION public.respuesta_xml(character varying)
...
begin
	...
	while (length(input1)>0) loop
		...
		if (strpos(input1,'</'||campo1||'>')>0) then
			...
			--Si alguna respuesta trae una <C> se reemplaza por el '
			out1:=out1||encode((previa1||'<'||campo1||'>')::bytea,'hex')||codifica_utf8(reemplaza_caracteres_web(replace(get_campo(upper(campo1),xml2),'<C>',chr(39))))||encode(('</'||campo1||'>')::bytea,'hex');
			--Avanzamos el input1.
