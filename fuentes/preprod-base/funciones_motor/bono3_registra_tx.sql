...
BEGIN
        ...
        --Guarda campo fecha_emision solo cuando es 'envbonis' o 'envbono'.
        ...
        BEGIN
                EXECUTE 'INSERT INTO '||tx_tabla1||'(tipo_tx, rut_benef, codigo_motor, fecha_in_tx, cod_fin, financiador, estado, codigo_resp,mensaje_resp, mensaje_fin, xml_input, sp_exec, host, folio, procesador_xml, conexiones_activas, fecha_emision, monto_aporte_total) VALUES ('||quote_literal(tipo_tx1)||', '||quote_literal(rut_ben1)||', '||get_campo('CODIGO_MOTOR',xml2)::bigint||', '||quote_literal(fecha_in_tx1)||', '||id_fin1||', '||quote_literal(get_campo('FINANCIADOR',xml2))||', '||quote_literal(get_campo('ESTADO_TX',xml2))||', '||quote_literal(get_campo('CODIGO_RESP',xml2))||', '||quote_literal(substring(get_campo('MENSAJE_RESP',xml2),1,50))||', '||quote_literal(substring(get_campo('ERRORMSG',xml2),1,50))||', '||quote_literal(xml_std1)||', '||quote_literal(get_campo('SQLINPUT',xml2))||', '||quote_literal(get_campo('__HOST__',xml2))||', '||quote_literal(folio1)||', '||quote_literal(get_campo('__PROCESOXML__',xml2))||', '||quote_literal(get_campo('__PROC_ACTIVOS__',xml2))||', '||fecha_emision1||', '||monto_aporte_total1||');';

                RETURN xml2;

        -- Si la tabla no existe, entonces la crea.
        EXCEPTION WHEN OTHERS THEN
                EXECUTE 'CREATE TABLE '||tx_tabla1||'(cod_fin integer, financiador varchar(50), tipo_tx varchar(20), codigo_motor bigint, rut_benef varchar(12), fecha_in_tx timestamp, fecha_out_tx timestamp, fecha_in_fin timestamp, fecha_out_fin timestamp, estado varchar(20), codigo_resp varchar(1), mensaje_resp varchar(50), mensaje_fin varchar(50), xml_input varchar, xml_output varchar, sp_exec varchar, sp_return varchar, host varchar(100), folio varchar, procesador_xml varchar, conexiones_activas varchar, fecha_emision integer, monto_aporte_total integer, dia integer DEFAULT(to_char(now(), ''YYYYMMDD''::text))::integer);';
