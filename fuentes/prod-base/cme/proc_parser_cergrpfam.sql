CREATE OR REPLACE FUNCTION proc_parser_input_cergrpfam(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	xml2:=  put_campo(xml2,'ANAMCODPARENT',		coalesce(nullif(get_campo('ANAMCODPARENT',xml2),''),'0'));
	xml2:=  put_campo(xml2,'ANAMANTPAR',		coalesce(nullif(get_campo('ANAMANTPAR',xml2),''),'0'));
