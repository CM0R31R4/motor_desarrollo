CREATE OR REPLACE FUNCTION traductor_in_cerdiagposop_masvida(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	xml2:=proc_parser_input_cerdiagposop(xml2);
	...
	exec_sp:= 'EXECUTE dbo.cerdiagposop '||get_campo('PROTIPODET',xml2)||',
			...
			['||get_campo('PROCODDIAGPOST',xml2)||'],
			['||get_campo('PROGLODIAGPOST',xml2)||'], @ProCodError OUTPUT, @ProMensajeError OUTPUT ';
...
CREATE OR REPLACE FUNCTION traductor_out_cerdiagposop_masvida(varchar)
returns varchar as
$$
declare
        xml1		alias for $1;
...
begin
	...
	raise notice 'JCC_cerdiagposop_RECEIVE ProCodError=% - ProMensajeError=%',cod_resp1,msj_resp1;
