CREATE OR REPLACE FUNCTION proc_parser_input_cerenvdetinsumo(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	xml2:=  put_campo(xml2,'CTAFECATENCION',coalesce(nullif(get_campo('CTAFECATENCION',xml2),''),'0'));
	xml2:=  put_campo(xml2,'CTAHORAATENCION',coalesce(nullif(get_campo('CTAHORAATENCION',xml2),''),'0'));
	xml2:=  put_campo(xml2,'CTATIPODET',	coalesce(nullif(get_campo('CTATIPODET',xml2),''),'0'));
	...
	xml2:=  put_campo(xml2,'CTACODINSUMO',	coalesce(nullif(get_campo('CTACODINSUMO',xml2),''),'0'));
	xml2:=  put_campo(xml2,'CTADESCINSUMO',	coalesce(nullif(get_campo('CTADESCINSUMO',xml2),''),'0'));
	xml2:=  put_campo(xml2,'CTACODINSUMOHOM',coalesce(nullif(get_campo('CTACODINSUMOHOM',xml2),''),'0'));
	xml2:=  put_campo(xml2,'CTACANTIDADINS',coalesce(nullif(get_campo('CTACANTIDADINS',xml2),''),'0'));
