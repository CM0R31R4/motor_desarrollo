CREATE OR REPLACE FUNCTION traductor_in_leerutcotiz_cruzdelnorte(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	xml2:=put_campo(xml2,'SQLINPUT','["ISAPRE_ADM.CDNLEERUTCOTIZ_PKG.CDNLEERUTCOTIZ", [ "$o$STRING", '||cod_fin1||','||chr(34)||rut_cotiz1||chr(34)||',"$o$STRING", "$o$STRING", "$o$STRING", "$o$NUMBER['||l_array||']", "$o$STRING['||l_array||']","$o$STRING['||l_array||']", "$o$STRING['||l_array||']", "$o$STRING['||l_array||']", "$o$STRING['||l_array||']", "$o$STRING['||l_array||']"]]'||chr(10)||chr(10));
...
CREATE OR REPLACE FUNCTION traductor_out_leerutcotiz_cruzdelnorte(varchar)
returns varchar as
$$
declare
        xml1		alias for $1;
	...
	resp1:=replace(get_campo('SQLOUTPUT',xml2),chr(10),'');
        ...
        if aux1 is null then
                ...
                --Otro Error
                        ...
                        xml2:=put_campo(xml2,'ERRORCOD','99');
			xml2:=put_campo(xml2,'ERRORMSG','CruzDelNorte:Error_Leerutcotiz');
                end if;
                xml2    :=put_campo(xml2,'CODIGO_RESP','2');
