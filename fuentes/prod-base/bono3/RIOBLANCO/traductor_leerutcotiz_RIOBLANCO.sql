CREATE OR REPLACE FUNCTION traductor_in_leerutcotiz_rioblanco(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	--Valida formato del Rut
        ...
        xml2:=put_campo(xml2,'SQLINPUT','["ADMIMED.RBLLEERUTCOTIZ_PKG.RBLLEERUTCOTIZ", [ "$o$STRING", '||chr(34)||cod_fin1||chr(34)||', '||chr(34)||rut_cotiza1||chr(34)||',"$o$STRING", "$o$STRING", "$o$STRING", "$o$NUMBER['||l_array||']", "$o$STRING['||l_array||']","$o$STRING['||l_array||']", "$o$STRING['||l_array||']", "$o$STRING['||l_array||']", "$o$STRING['||l_array||']", "$o$STRING['||l_array||']"]]'||chr(10)||chr(10));
...
CREATE OR REPLACE FUNCTION traductor_out_leerutcotiz_rioblanco(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	if aux1 is null then
		...
		if aux1 is not null then
                        ...
                        xml2:=put_campo(xml2,'ERRORCOD','99');
			xml2:=put_campo(xml2,'ERRORMSG','RioBlanco:Error_Leerutcotiz');
                end if;
                xml2    :=put_campo(xml2,'CODIGO_RESP','2');
