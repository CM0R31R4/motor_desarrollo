CREATE OR REPLACE FUNCTION bono3.traductor_in_bencertif_rioblanco(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	if length(rut_benef1)>'0' then rut_benef1:=lpad(rut_benef1,10,'0')||'-'||motor_modulo11(rut_benef1);end if;
        ...
        xml2:=put_campo(xml2,'INPUT','["IRB_ADMIMED.RBLBENCERTIF_PKG.RBLBENCERTIF", [ "$o$STRING", '||cod_fin1||','||chr(34)||rut_benef1||chr(34)||','||chr(34)||extFechaActual||chr(34)||', "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$NUMBER", "$o$STRING[10]", "$o$STRING[40]" ]]'||chr(10)||chr(10));
...
CREATE OR REPLACE FUNCTION bono3.traductor_out_bencertif_rioblanco(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	--Si es un beneficiario certificado
        ...
        xml2:=put_campo(xml2,'EXTNOMBREACOMPANANTE',    trim(replace(json_field(aux1,'19'),'"','')));
