CREATE OR REPLACE FUNCTION traductor_in_valorvari_ferrosalud(varchar)
returns varchar as
$$
declare
...
CREATE OR REPLACE FUNCTION traductor_out_valorvari_ferrosalud(varchar)
returns varchar as
$$
declare
	xml1  	alias for $1;
...
begin
	...
	xml2    :=put_campo(xml2,'ESTADO_TX','TERMINADO_OK');
	 ...
	 --Si viene al menos uno de estos, obtiene 1er grupo reg.
                	...
                	--En el sgte ciclo, solo vienen 5 iteraciones. Obteniendo dos registros x cada iteracion
                        while (j<6) loop
                                ...
                                if length(bonif_aux1)>0 or length(ext_cop_aux1)>0 then
                                        ...
                                        xml2:=logapp(xml2,'FERROSALUD: RSP_VALORVARI -> Falla Obtencion 2doGrupo del Recordset - CodMotor= '||get_campo('CODIGO_MOTOR',xml2));
                                end if;
        ...
        xml2:=logapp(xml2,'FER_ValorVari_Receive extcoderror ='||cod_resp1||' extmensajeerror ='||mensaje_resp1||' cont i='||i||' cont j='||j); 
