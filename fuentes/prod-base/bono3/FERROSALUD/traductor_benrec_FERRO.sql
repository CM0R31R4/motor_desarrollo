CREATE OR REPLACE FUNCTION traductor_in_benrec_ferrosalud(varchar)
returns varchar as
$$
declare
        xml1    	alias for $1;
...
begin
	...
	--Valida formato del Rut
        ...
        xml2:=put_campo(xml2,'SQLINPUT',declare_params ||' EXECUTE dbo.FERBenRec '|| cod_fin1 ||',"'||rut_cotiza1||'",'||corr_benef1||',"'||rut_benef1||'",'||output_params);
	return xml2;
end;
$$
LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION traductor_out_benrec_ferrosalud(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	xml2:=logapp(xml2,'FERROSALUD: RSP_BENREC -> extCodResBen='||cod_resp1||' -extMensajeError='||mensaje_resp1);
