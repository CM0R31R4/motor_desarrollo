CREATE OR REPLACE FUNCTION traductor_in_enrola_ferrosalud(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	exec_sp:= 'execute dbo.FEREnrola '||cod_fin1||',"'||rut_enrola1||'","'||rut_benef1||'", @extValido OUTPUT, @extNombreComp OUTPUT; ';
	/*Ejemplo SP SYBASE*/
	--exec dbo.FEREnrola 88, '0006456149-9', '0005308213-0', @extValido output, @extNombreComp output
...
CREATE OR REPLACE FUNCTION traductor_out_enrola_ferrosalud(varchar)
returns varchar as
$$
declare
        xml1		alias for $1;
...
begin
	...
	xml2:=logapp(xml2,'FERROSALUD: RSP_ENROLA -> extValido='||cod_resp1||' -extNombreComp='||nombre1);
