CREATE OR REPLACE FUNCTION traductor_in_leerutcotiz_ferrosalud(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	exec_sp:= 'execute dbo.FERLeeRutCotiz '||cod_fin1||',"'||rut_cotiz1||'", @extNomCotizante OUTPUT, @extCodError OUTPUT, @extMensajeError OUTPUT; ';
	/*Ejemplo*/ 
	--exec_sp:= dbo.FERLeeRutCotiz 88, '0009702357-3', @extNomCotizante output, @extCodError output, @extMensajeError output;
...
CREATE OR REPLACE FUNCTION traductor_out_leerutcotiz_ferrosalud(varchar)
returns varchar as
$$
declare
        xml1		alias for $1;
...
begin
	...
	while true loop
		corr_aux1	:=trim(get_campo('BNF_CODIGO_'||i::varchar,xml2));	--EXTCORRBENEF
		rut_aux1	:=trim(get_campo('BNF_RUT_'||i::varchar,xml2));		--EXTRUTBENEFICIARIO
		pat_aux1	:=trim(get_campo('BNF_PATERNO_'||i::varchar,xml2));	--EXTAPELLIDOPAT
		mat_aux1	:=trim(get_campo('BNF_MATERNO_'||i::varchar,xml2));	--EXTAPELLIDOMAT
		nom_aux1	:=trim(get_campo('BNF_NOMBRES_'||i::varchar,xml2));	--EXTNOMBRES
		cod_aux1	:=trim(get_campo('BNF_CODEST_'||i::varchar,xml2));	--EXTCODESTBEN
		desc_aux1    	:=trim(get_campo('BNF_NOMEST_'||i::varchar,xml2));	--EXTDESCESTADO
	...
	xml2:=logapp(xml2,'FERROSALUD: RSP_LEERUTCOTIZ -> extNomCotizante='||nombre_cotiz1||' -extCodError='||cod_resp1||' -extMensajeError='||mensaje_resp1||' -extCorrBenef='||corr_benef1||' -extRutBeneficiario='||rut_benef1||' -extApellidoPat='||apell_pat1||' -extApellidoMat='||apell_mat1||' -extNombres='||nombres1||' -extCodEstBen='||cod_est_benef1||' -extDescEstado='||desc_estado1);
