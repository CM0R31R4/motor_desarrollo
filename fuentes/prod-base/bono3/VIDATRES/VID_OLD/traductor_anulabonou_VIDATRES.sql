CREATE OR REPLACE FUNCTION bono3.traductor_in_anulabonou_vidatres(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	extCodFinanciador       := trim(get_campo('EXTCODFINANCIADOR',xml2));
        ...
        xml2:=put_campo(xml2,'INPUT','["INFOMEDICA.VIDANULABONOU_PKG.VIDANULABONOU", [ "$o$STRING", '||extCodFinanciador||', '||extFolioBono||', '||chr(34)||extIndTratam||chr(34)||', '||chr(34)||extFecTratam||chr(34)||', "$o$STRING", "$o$STRING"]]'||chr(10)||chr(10));
...
CREATE OR REPLACE FUNCTION bono3.traductor_out_anulabonou_vidatres(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
