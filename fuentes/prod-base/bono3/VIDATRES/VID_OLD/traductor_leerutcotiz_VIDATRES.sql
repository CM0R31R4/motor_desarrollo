CREATE OR REPLACE FUNCTION bono3.traductor_in_leerutcotiz_vidatres(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	extCodFinanciador       := trim(get_campo('EXTCODFINANCIADOR',xml2));
        ...
        --Rut con formato
        --rut_cotiza1     :=lpad(rut_cotiza1,10,'0');
        ...
        xml2:=put_campo(xml2,'INPUT','["INFOMEDICA.VIDLEERUTCOTIZ_PKG.VIDLEERUTCOTIZ", [ "$o$STRING", '||extCodFinanciador||', '||chr(34)||rut_cotiza1||chr(34)||',"$o$STRING", "$o$STRING", "$o$STRING", "$o$NUMBER[10]", "$o$STRING[10]","$o$STRING[10]", "$o$STRING[10]", "$o$STRING[10]", "$o$STRING[10]", "$o$STRING[10]"]]'||chr(10)||chr(10));
...
CREATE OR REPLACE FUNCTION bono3.traductor_out_leerutcotiz_vidatres(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
