CREATE OR REPLACE FUNCTION traductor_in_enrola_colmena(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	exec_sp:= 'EXECUTE CGCEnrola '||cod_fin1||',"'||rut_enrola1||'","'||rut_benef1||'",@extValido OUTPUT,@extNombreComp OUTPUT ';
	/*Ejemplo SP SYBASE*/
	--exec CGCEnrola 78, '0006456149-9', '0005308213-0', @extValido output, @extNombreComp output
...
CREATE OR REPLACE FUNCTION traductor_out_enrola_colmena(varchar)
returns varchar as
$$
declare
        xml1		alias for $1;
...
begin
	...
	xml2:=logapp(xml2,'COLMENA: RSP_ENROLA -> extValido='||cod_resp1||' -extNombreComp='||nombre1);
