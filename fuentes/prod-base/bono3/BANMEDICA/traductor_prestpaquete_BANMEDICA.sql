CREATE OR REPLACE FUNCTION traductor_in_prestpaquete_banmedica(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	xml2:=put_campo(xml2,'SQLINPUT','["INFOMEDICA.BANPRESTPAQUETE_PKG.BANPRESTPAQUETE", [ "$o$STRING", '||cod_fin1||','||chr(34)||num_conv1||chr(34)||','||chr(34)||lugar_conv1||chr(34)||','||chr(34)||cod_paquete1||chr(34)||', "$o$STRING","$o$STRING","$o$STRING['||l_array||']","$o$STRING['||l_array||']","$o$NUMBER['||l_array||']" ]]'||chr(10)||chr(10));
...
CREATE OR REPLACE FUNCTION traductor_out_prestpaquete_banmedica(varchar)
returns varchar as
$$
declare
        xml1		alias for $1;
...
begin
	...
	resp1:=replace(get_campo('SQLOUTPUT',xml2),chr(10),'');
        ...
        if aux1 is null then
                ...
                --Otro Error
                        ...
                        xml2:=put_campo(xml2,'ERRORCOD','99');
			xml2:=put_campo(xml2,'ERRORMSG','Banmedica:Error_PrestPaquete');
                end if;
                xml2    :=put_campo(xml2,'CODIGO_RESP','2');
	...
	xml2:=logapp(xml2,'BANMEDICA: RSP_PRESTPAQUETE -> extCodError='||cod_resp1||' -extMensajeError='||mensaje_resp1||' -extCodHomologo='||cod_homol1||' -extItemHomologo='||item_homol1||' -extCantidad='||cantidad1);
