        ...
        while true loop
			...
			--En el sgte ciclo, solo vienen 5 iteraciones. Obteniendo dos registros x cada iteracion 
					...
					xml2:=put_campo(xml2,'EXTTIPOBONIF'||j::varchar,bonif_aux1);
      					...
      					xml2:=logapp(xml2,'CRUZBLANCA: RSP_VALORVARI -> Falla Obtencion 2doGrupo del Recordset - CodMotor= '||get_campo('CODIGO_MOTOR',xml2));
                                end if;
