CREATE OR REPLACE FUNCTION bono3.traductor_in_bencertif_consalud(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	cod_fin1        :=trim(get_campo('EXTCODFINANCIADOR',xml2));
        ...
        --fecha_actual1   :=to_char(get_campo('EXTFECHAACTUAL',xml2)::date, 'DD/MMMM/YYYY');
	...
	xml2:=put_campo(xml2,'SQLINPUT','["IMEDSOF.CONBENCERTIF_PKG.CONBENCERTIF", [ "$o$STRING", '||cod_fin1||','||chr(34)||rut_ben1||chr(34)||','||chr(34)||fecha_actual1||chr(34)||', "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$STRING", "$o$NUMBER", "$o$STRING['||l_array||']", "$o$STRING['||l_array||']" ]]'||chr(10)||chr(10));
...
CREATE OR REPLACE FUNCTION bono3.traductor_out_bencertif_consalud(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	--Si no esta el tag result es porque hay error
        ...
        if aux1 is null then
                ...
                --Otro Error
                        ...
                        xml2:=put_campo(xml2,'ERRORCOD','99');
			xml2:=put_campo(xml2,'ERRORMSG','Consalud:Error_BenCertif');
                end if;
                xml2    :=put_campo(xml2,'CODIGO_RESP','2');
	...
	raise notice 'JCC_BENCERTIF_Receive extApellidoPat=% - extApellidoMat=% - extNombres=% - extSexo=% - extFechaNacimi=% - extCodEstBen=% ',apell_pat1,apell_mat1,nombre1,sexo1,fecha_nac1,cod_est_ben1;

        cod_est_ben1    :=trim(replace(json_field(aux1,'6'),'"',''));
	 --Beneficiario Certificado
        if (cod_est_ben1='C') or (cod_est_ben1='G') or (cod_est_ben1='S') then
	        --Resp OK fecha_nac viene asi "198203230000000000000"
                fecha_nac1:=substring(fecha_nac1,1,8);
