CREATE OR REPLACE FUNCTION bono3.traductor_in_benrec_consalud(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	if length(rut_cotiza1)>'0' then rut_cotiza1     :=lpad(rut_cotiza1,10,'0')||'-'||motor_modulo11(rut_cotiza1);end if;
        ...
        xml2:=put_campo(xml2,'INPUT','["IMEDSOF.CONBENREC_PKG.CONBENREC", [ "$o$STRING", '||extCodFinanciador||', '||chr(34)||rut_cotiza1||chr(34)||', '||ExtCorrBenef||', '||chr(34)||rut_benef1||chr(34)||', "$o$STRING", "$o$STRING"]]'||chr(10)||chr(10));
...
CREATE OR REPLACE FUNCTION bono3.traductor_out_benrec_consalud(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
