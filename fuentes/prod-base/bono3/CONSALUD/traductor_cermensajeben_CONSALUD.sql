CREATE OR REPLACE FUNCTION traductor_in_cermensajeben_consalud(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	--Valida formato del Rut
        ...
        xml2:=put_campo(xml2,'SQLINPUT','["IMEDSOF.CONMENSAJEBEN_PKG.CONMENSAJEBEN_PKG", [ "$o$STRING", '||cod_fin1||', '||chr(34)||rut_ben1||chr(34)||', "$o$NUMBER", "$o$STRING","$o$STRING" ]]'||chr(10)||chr(10));
...
CREATE OR REPLACE FUNCTION traductor_out_cermensajeben_consalud(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	resp1	=replace(get_campo('SQLOUTPUT',xml2),chr(10),'');
        ...
        if aux1 is null then
                ...
                --Otro Error
                        ...
                        xml2:=put_campo(xml2,'ERRORCOD','99');
			xml2:=put_campo(xml2,'ERRORMSG','Consalud:Error_CerMensajeBen');
                end if;
                xml2    :=put_campo(xml2,'CODIGO_RESP','2');
	...
	xml2:=logapp(xml2,'CONSALUD: RSP_CERMENSAJEBEN -> extTipoAccion='||tipo_accion1||' -extMsgError1='||msg_error1||' --extMsgError2='||msg_error2);
