CREATE OR REPLACE FUNCTION traductor_in_leerutcotiz_fundacion(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	--Setea Timeout de la base de datos
        xml2    :=put_campo(xml2,'__TIMEOUT_BD_PXML__','21');
	...
	xml2:=put_campo(xml2,'SQLINPUT','["ADMIMED.FNDLEERUTCOTIZ_PKG.FNDLEERUTCOTIZ", [ "$o$STRING", '||cod_fin1||', '||chr(34)||rut_cotiza1||chr(34)||', "$o$STRING", "$o$STRING", "$o$STRING", "$o$NUMBER['||l_array||']", "$o$STRING['||l_array||']","$o$STRING['||l_array||']", "$o$STRING['||l_array||']", "$o$STRING['||l_array||']", "$o$STRING['||l_array||']", "$o$STRING['||l_array||']"]]'||chr(10)||chr(10));
...
CREATE OR REPLACE FUNCTION traductor_out_leerutcotiz_fundacion(varchar)
returns varchar as
$$
declare
        xml1    alias for $1;
...
begin
	...
	if aux1 is null then
		...
		if aux1 is not null then
                        ...
                        xml2:=put_campo(xml2,'ERRORCOD','99');
			xml2:=put_campo(xml2,'ERRORMSG','Fundacion:Error_LeeRutCotiz');
                end if;
                xml2    :=put_campo(xml2,'CODIGO_RESP','2');
