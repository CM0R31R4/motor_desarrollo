...
begin
		...
		--Nunca se actualiza el INPUT original de la cola
		xml3:=put_campo(xml2,'_LOG_','');
                xml2:=logapp(xml2,'Aumenta Reintentos');
                execute 'update confirma_bono_'||cola1||' set estado='||quote_literal(get_campo('STATUS_BONO',xml2))||',reintentos=reintentos+1,xml_in='||quote_literal(xml3)||' where id='||id1;
                --execute 'update confirma_bono_'||cola1||' set estado='||quote_literal(get_campo('STATUS_BONO',xml2))||',reintentos=reintentos+1 where id='||id1;
                return xml2;
	end if;
