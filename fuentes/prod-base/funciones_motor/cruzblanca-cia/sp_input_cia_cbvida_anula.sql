...
*/
CREATE OR REPLACE FUNCTION sp_input_cia_cbvida_anula(character varying)
...
begin
        ...
        --Log del input
        xml2:=logapp(xml2,'Input.cbvida.Anula='||input1);
        ...
        respuesta1:=respuesta1||'<ns1:extFolioAuto>'||get_xml('extFolioAuto',input1)||'</ns1:extFolioAuto>';
        --respuesta1:=respuesta1||'<ns1:extFolioBono>'||get_xml('extFolioBono',input1)||'</ns1:extFolioBono>';
        --respuesta1:=respuesta1||'</MsgInput>'||chr(10)||']]>';
        --Cambio los caracteres < y >
        xml2:=put_campo(xml2,'DATA_CIA',respuesta1);

        xml2:=put_campo(xml2,'CIA_SOAPACTION','http://Seguros.CruzBlanca.cl/WsIntegracionLGM/Anulacion');
        return xml2;
end;
$function$
