...
Tipo_XML *InsertaDataLenXML1(Tipo_XML *xml,char szCampo[],char szData[],int nTipoCampo,int nLenData)
{
	Tipo_XML *xml1=xml;
	Tipo_XML *xml2=NULL;
	Tipo_XML *xml_up;
...
{
	return InsertaDataLenXML1(xml,szCampo,szData,0,nLenData);
}
...
{
	return InsertaDataLenXML1(xml,szCampo,szData,0,strlen(szData));
}
...
Tipo_XML *LeePaqueteSCGI(int id,int socket,int nTimeout,Tipo_XML *xml,int *nStatus)
{
	...
	break;
     ...
     i=0;
     sprintf(szAux,"lLenData=%ld nLen==%ld",lLenData,nLen);
     WriteLog(id,szAux);
     for(j=0;j<lLenData;j++)
     { 
     	sts=recv(socket,szLinea,1,0);
	if (sts<0)
     	{	
...
int LeePaqueteBD(int id,int socket,char szData[],int nMaxLen,int nTimeout)
{
	...
	if (nLeer>MAX_BUFFER)
	{
			...
			send(socket,"BUFFER MUY LARGO",16,0);
			return CLOSE_SOCKET;
	}
	szData[0]=0;
	j=0;
...
int SendDatabase(int id,char szSql[])
{
	...
	else xml=QueryDatabase1(id,szSql,"SQL",szError,xml,&nStatus);
	xml=CierraXML(xml);
...
void WriteMensajeApp(int id,char *data)
{
        ...
        fprintf(f,"%02i%02i%02i%03i <%02i> %s\n",thoy.tm_hour,thoy.tm_min,thoy.tm_sec,sTimeb.millitm,id,data);
...
void WriteLog(int id,char *szMessage)
{
	...
	// write the file
	//fprintf(f,"%04i%02i%02i,%02i%02i%02i%03i <%02i> %s\n",thoy.tm_year,thoy.tm_mon+1,thoy.tm_mday,thoy.tm_hour,thoy.tm_min,thoy.tm_sec,sTimeb.millitm,id,szMessage);
	fprintf(f,"%02i%02i%02i%03i <%02i> %s\n",thoy.tm_hour,thoy.tm_min,thoy.tm_sec,sTimeb.millitm,id,szMessage);
	}
	else
	{
		fprintf(f,"%02i%02i%02i%03i <%02i> ",thoy.tm_hour,thoy.tm_min,thoy.tm_sec,sTimeb.millitm,id);
...
otra_hh_asin:
	...
	if (bAsincrona)
	{
		...
		if (pPaquete) sts=send(pSocketServ->m_socket,pPaquete->data,pPaquete->nLenData,0);
