...
int ProcesaTransaccion(int id,int nPosLlave,char szData[],int nTipoXml,int nLenData)
{
	...
	if (memcmp(stServ.szEscribeFile,"SI",2)==0)
	{
		...
		*/
		...
		free(pDataHex);
#ifdef FLAG_POSTGRES
		pSqlResp=GetOneRecordAsinc1(id,pSql->data,pSqlResp);
		if (pSqlResp==NULL) 
		{
		...
		send(nSocket,szResp,strlen(szResp),0);
	...
	else
	{
		...
		else send(nSocket,szResp,nLenResp,0);
	}
...
error:
	...
	else send(nSocket,szResp,strlen(szResp),0);
	//xml1=CierraXML(xml1);
	xml=CierraXML(xml);
	return 1;
}
...

{
	...
	if (!ConectaCliente(id,nPort,szIp,&stSocketServicio))
	{
		...
		else send(socket,response,strlen(response),0);
		return 0;
	}
	...
	else if (GetStrXML(xml,"HTML",szAux,sizeof(szAux))>0)
	{
		...
		else send(socket,szAux,strlen(szAux),0);
	}
	...
	else
	{
		...
		else send(socket,szPaqueteXML,strlen(szPaqueteXML),0);
	}
	xml=CierraXML(xml);
	}while(bNext);
fin:
