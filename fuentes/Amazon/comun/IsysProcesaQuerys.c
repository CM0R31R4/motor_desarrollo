	...
	*nProxSec=pQuerys->nSecError;
	...
	WriteLog(id,szDirectorio);
       ...
       if (!S_ISDIR(buf_stat.st_mode))
       {
            ...
            sprintf(szLinea,"mkdir %s",szDirectorio);
            fp = popen(szLinea, "r");
            if (fp == NULL) WriteLog(id,"Falla Ejecutar Comando");
            else pclose(fp);
            stat(szDirectorio,&buf_stat);
		  ...
		  return xml;
            ...
            else WriteLog(id,"Dir Creado OK");
	}
	...
	if (nLen>0)
	{
		...
		sprintf(szError,"Bytes Escritos %i",nLen);
		WriteLog(id,szError);
	}
	fclose(f);
...
Tipo_XML *AplicaQuerys(int id,int nSocket,Tipo_XML *xml,Tipo_Querys *pQueryOrg,int *sts,int *bContesta)
{
	...
	*bContesta=0;
	...
	do
	{
		...
		if (global.nError)
		{
			xml=InsertaDataXML(xml,"ERROR",global.szError);
			global.nError=0;
		}
...
Tipo_XML *AplicaQuerysServicio(int id,Tipo_XML *xml,int nTx,int *bContesta,int *nStatus)
{
	...
	*nStatus=0;
	...
	ImprimeMemoria(id,"Despues de LeeQuery");
	xml=AplicaQuerys(id,nSocket,xml,pQuerys,&sts,bContesta);
	ImprimeMemoria(id,"Despues de AplicaQuerys");
	*nStatus=1;
	return xml;
}
