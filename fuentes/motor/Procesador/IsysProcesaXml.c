...
Tipo_XML *AsignaDatosInicio(int id,Tipo_XML *xml)
{
	...
	sprintf(szAux,"%i",global.procesos_activos);
        xml=InsertaDataXML(xml,"__PROC_ACTIVOS__",szAux);
}
...
volver_scgi:
		...
		if (sts>0)
		{
			...
			{
				bContesta=0;
				//Verificamos que el largo de la data no supere el largo de stServ.szUrl
				if (xml_aux->nLenData>sizeof(stServ.szUrl))
				{
					WriteLog(id,"REQUEST_URI Larga, se busca hasta 50 caracteres");
					WriteMensajeApp(id,"REQUEST_URI Larga, se busca hasta 50 caracteres");
					memcpy(stServ.szUrl,xml_aux->pData,sizeof(stServ.szUrl));
					stServ.szUrl[sizeof(stServ.szUrl)]=0;
				}
				else sprintf(stServ.szUrl,"%s",xml_aux->pData);
				if (!BuscaServicio(id,&stServ,BUSCA_URL))
				{
					...
					WriteMensajeApp(id,stServ.szUrl);
					close(nSocket);
					xml=CierraXML(xml);
					goto fin;
				}
	...
	*/
	...
	//En caso de que el socket fuera cerrado por la ejecucion de la tx, volvmoes a asginarlo
	nSocket=Conexion[id].nSocket;
	//ImprimeXML1(xml,id);
	//Si esta activado el scgi, solo responde tag RESPUESTA si el socket esta activo
        if (global.scgi==1) 
	{
		if (nSocket>0) 
		{
	...
	else if (Conexion[id].nTipoProceso==PROC_SOCKET)
 	{
		...
		else WriteLog(id,"Socket ya Cerrado*");
		xml=CierraXML(xml);
	}
	else if (Conexion[id].nTipoProceso==PROC_XML)
	{
