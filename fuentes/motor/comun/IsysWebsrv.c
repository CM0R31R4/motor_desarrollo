...
int RetornaPagina(int id,char szComando[],int socket)
{
	...
	if (!BuscaPagina(id,szComando,szPagina))
	{
		...
		send(socket,szError,strlen(szError),MSG_NOSIGNAL );
		return 0;
	}
...
int SendHeaderHTTP(int id,int socket)
{
	...
	return send(socket,szData,strlen(szData),MSG_NOSIGNAL );
}
...
int EnviaPagina(int id,char szPagina[],int socket)
{
	...
	else send(socket,data_file,len_file,MSG_NOSIGNAL );
	printf(data_file);
	/*printf(data_file);
	...
	if (sts)
	{
		...
		*nLeidos=sts;
		...
		while(1)
		{
			...
			//Si es chunked ignoro la linea y le agrego un content length
			if (strstr(szLinea,"Transfer-Encoding: chunked")>0 || strstr(szLinea,"transfer-encoding: chunked")>0)
			{
			...
			{
				//Si es chunked no agregue el ultimo enter
				if (bChunked==1 && szLinea[0]==10)
				{	
					WriteLog(id,"No grabo ultimo CR en chunked");
				}
				else
				{
					pData=ConcatenaData(szLinea,pData,sts);
...
chunked:
		...
		if (bChunked)
		{
			...
			if (lLen==0) 
			{
				...
				//Agrego el Contente Length
				sprintf(szLinea,"Content-Length: %i\n\n",pDataChunked->nLenData);
				pData=ConcatenaData(szLinea,pData,strlen(szLinea));
				//Junto los header con la data chunked leida
				pData=ConcatenaData(pDataChunked->data,pData,pDataChunked->nLenData);
				*nLeidos=pData->nLenData;
				//Libero la data chunked
				pDataChunked=LiberaData(pDataChunked);
				return pData;
			}

			//Se Lee la data 	
			//pData=LeeNData_sin_CRLF(id,socket,lLen,pData,nTimeout,&sts);
			pDataChunked=LeeNData(id,socket,lLen,pDataChunked,nTimeout,&sts);
			if (sts>0)
			{	
				if (pData)
				{
					...
					//Ignoro la data 
					pAux=LeeNData(id,socket,2,pAux,nTimeout,&sts);
					//*nLeidos+=sts;
					//*nLeidos=pData->nLenData;
				}
				else
				{	
					...
					WriteLog(id,"Falla leer Chunked HTTP 1");
					pData=LiberaData(pData);
					*nLeidos=-1;
					return NULL;
				}
