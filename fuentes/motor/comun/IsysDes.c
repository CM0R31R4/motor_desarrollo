...
void endes(inblock,outblock)			/* encrypt 64-bit inblock	*/
...
void Encripta(char szLlave[],char szUsuario[],char szBuffer[])
{
	register int m;
	register char *b;
	char a[10];
	...
	memset(a,0,8);
	//Genero la llave de encriptacion..
	sprintf(szLlave1,"SGL2758.%s",szUsuario);
	//inicia tablas...
	desinit(szLlave1);	  /* set up tables for DES	*/

	sprintf(szLlave,"");
	m=strlen(szBuffer);
	//printf("Buffer=%s\n\r",szBuffer);

	for (b=szBuffer; m>0;m -= 8, b += 8)/* 8-byte blocks*/
	{
		...
		endes(b,a);	/* don't need diff input, output*/
		writehex(a,tmp);
		strcat(szLlave,tmp);
		//printf("Concatena = %s\n\r",tmp);
	}
...
int Desencripta(char szDes[],char szUsuario[],char szLlaveOut[])
{
	char tmp[20];
	char szTmp[512];
	char *dta;
	...
	m=strlen(szDes)/2;
	printf("Llave Real =%s\n\r",szTmp);
	memset(szLlaveOut,0,512);
	for (b=szTmp; m>0;m -= 8,b+= 8)
	{
		dedes(b,a);
		strncat(szLlaveOut,a,8);
	}
	printf("Llave Out = %s\n\r",szLlaveOut);
