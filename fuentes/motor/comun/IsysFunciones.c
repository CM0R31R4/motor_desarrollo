...
void readhex_39to5(char szHex[],char szout[])
{
    int i,j=0;
    int len;
    len=strlen(szHex);
    for (i = 0; i < len; i+=2)
    {
	  ...
	  //La comilla se remplaza por el caracter 5 ENQ para luego manajerla en el servicio
	  if (szout[j-1]==39)
		szout[j-1]=5;
	  //Por seguridad el ; se remplaza por el caracter 6 ACK para evitar el SQL injection
	  if (szout[j-1]==59)
		szout[j-1]=6;
    }
    szout[j++]=0;
	...
	*nNivel=-1;
	...
	for(i=0;i<p->nTotal;i++)
	{
		...
		case STRSTR:
			...
			//if (strstr(szAux,szData)>0 && strlen(szAux)>0) 
			{
				bState=1;
			}
			break;
...
void WriteHex(int id,char *szMessage,int nLen)
{
   ...
   //Si esta definido el largo de linea
   if (global.nLargoLineaLog!=-1) 
   {
	//Si nLen es meno a global.nLargoLineaLog, entonces no hago nada
	if (nLen>global.nLargoLineaLog) nLen=global.nLargoLineaLog;
   }
   ...
   while(j<nLen)
   {
	   ...
	   for(i=0;i<16;i++)
	   {
		...
		j++;
		//Si llegue a termino no sigo
		if (j==nLen) 
		{
			int z;
			//Lo relleno de espacios
			for(z=i+1;z<16;z++)
			{
				sprintf(szTmp,"   ");
				strcat(szBufferHex,szTmp);
